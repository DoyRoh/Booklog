...
- **`/api/ping` 진단 페이지(신규)**: "여전히 느리다, 리전을 다시 볼까"는 질문에 이 환경에서는 배포 주소로 나가는 연결이 막혀 직접 잴 수 없어서, 폰에서 열면 글자로 바로 보이는 진단 라우트를 넣었습니다 — Vercel 리전(`VERCEL_REGION`), Supabase 호스트, 이 서버 인스턴스가 콜드 스타트인지, 인증 확인(getClaims) ms, DB 왕복 1회 ms, 합계. 참고: 배포 직후엔 항상 콜드 스타트라 첫 화면 몇 개가 느린 게 정상이고, 이번 세션처럼 짧은 간격으로 여러 번 배포하면 테스트할 때마다 그걸 맞게 됩니다.

## 책 검색에서 "과학공룡"으로 "내친구 과학공룡"이 안 나오던 것 (사용자 지적)

`/api/books/lookup?query=`가 카카오 책 검색을 `size=10`으로 한 번만 불렀습니다. 카카오는 "과학공룡"을 과학+공룡으로 나눠 관련 책을 정확도순으로 주기 때문에, 제목에 그 말이 통째로 들어간 전집("내친구 과학공룡(2019개정판)(총105종)")이 10건 안에 못 들었습니다. 이제 **제목만 대상(target=title)**과 **전체(제목·저자·출판사)** 두 검색을 동시에 부르고(각 20건), ISBN/제목으로 중복을 없앤 뒤 **검색어(띄어쓰기 무시)가 제목에 그대로 들어간 책을 맨 위**로 올려 최대 30건을 돌려줍니다. 부모 기록 남기기의 제목 자동완성과 숲지기 책 찾기(`BookFinder`) 둘 다 같은 라우트를 쓰므로 함께 좋아집니다. **전집이 "세트(총N종)"로만 나오는 건 카카오(다음 책) 데이터의 한계**입니다 — 낱권에 따로 ISBN이 붙어 유통되지 않으면 검색에 낱권이 없습니다. 낱권은 "검색에 안 나오는 책이에요"로 제목을 직접 넣거나, 책 뒷면 바코드(낱권 ISBN이 있으면)를 찍으면 됩니다. 낱권까지 잘 잡히는 데이터(알라딘 API 등)로 바꾸는 건 별도 작업.

## 숲지기 추천도서 탭 = 그룹별 미리보기 10권, 관리는 그룹별 화면에서 (사용자 요청)
...
## 우리 숲 3D (사용자 요청: "모은 배지(나무·등불·버섯…)가 3D 공간으로 구현돼서 아이가 모아 가는 재미를 느끼게")

- **`/forest/3d`(신규, `app/forest/3d/page.tsx` + `components/forest-3d-screen.tsx` + `components/forest-3d.tsx`)**: 우리 숲(`/forest`) 제목 옆 "3D 숲 ›"(빈 숲에서도 "빈 들판을 3D로 둘러보기 ›")로 들어가는 화면. 데이터는 평면 숲과 같은 `loadBadges()` 하나. `three` + `@react-three/fiber` + `@react-three/drei`를 새로 넣었고, 이 화면에서만 `next/dynamic({ ssr: false })`로 불러 다른 화면 번들엔 안 실린다.
- **`lib/forest-scene.ts`(신규)**: 평면 숲(`forest-view.tsx`)에 있던 "배지 → 장식" 표(`ORNAMENT_BY_BADGE`, `ORNAMENT_LABEL`)와 나무 종류 규칙을 여기로 옮겨 두 화면이 공유. `layoutForest(badges)`가 나무·장식을 섞어 **해바라기 나선(황금각)** 자리에 놓는다 — 첫 나무가 가운데 가까이, 배지를 딸수록 숲이 바깥으로 넓어져 "모아 가는" 느낌이 공간으로 보인다. 3D에서는 발자국 배지가 **버섯**(`ORNAMENT_LABEL_3D`)으로 자란다.
- **그림 파일 없이 기본 도형으로 찰흙(클레이) 느낌**: 공·원뿔·원기둥·캡슐만으로 나무 4종(연한·풍성한·둥근·전나무, 권수 구간별 크기), 별(떠서 흔들림), 등불(반투명 호박 유리 + 발광 구, 밤엔 앞 세 개만 진짜 광원), 편지 물고 도는 백로, 그루터기에 앉은 새, 빨간 버섯, 그리고 숲길 시작의 **아이 아바타(토끼/강아지/고양이)와 등불 든 반달곰**(몸 공+머리 공, 목 없음, 점 눈, 가슴 흰 초승달 — 2D 캐릭터 규칙 그대로). 재질은 전부 무광(`roughness 1`), 색은 디자인 토큰 팔레트. 나중에 진짜 GLB 모델이 생기면 `forest-3d.tsx`의 조각만 바꿔 끼우면 된다.
- **조작**: 손가락으로 돌리기·벌려서 확대(OrbitControls, 이동 없음, 각도·거리 제한), 자동 회전, 나무·장식을 누르면 위 카드에 어떤 배지인지(호박색 링 표시), 낮/밤 토글(밤엔 별하늘·등불이 길을 비춤). 움직임 최소화 설정이면 자동 회전·애니메이션 끔. iPhone 뷰포트에서 낮/밤 스크린샷으로 확인(임시 라우트, 삭제). DB 변경 없음.
- **처음에 놓친 것**: 등불 집을 불투명 상자로 만들어 안의 발광 구가 완전히 가려져 밤에 안 켜져 보였다(스크린샷으로 발견) → 반투명 유리 + 네 모서리 기둥으로. 곰의 초승달은 ∩ 방향이라 찡그린 입처럼 보여 U 방향으로.

## 숲의 곰·백로가 속한 그룹 수만큼 (사용자 요청: "곰이나 백로 같은 새는 속한 그룹 늘어나면 늘려줘")
...
## 하단 탭에 "추가" 버튼 (사용자 요청: "책장에 책 꽂는 메뉴도 하나 추가하자")

부모 하단 탭(오늘·책장·숲길·숙제)에 **책장 바로 다음**으로 "추가"(원 안의 +, `AddIcon`)를 넣었습니다 — `/library/add`(기록 남기기 화면)로 바로 이어지고, 여기서 책을 찾아 저장하면 그 즉시 책장에 꽂힙니다. 숲지기 탭(대시보드·아이들·추천도서·숙제)은 각 화면에 이미 "+ 추가" 진입점이 따로 있어 건드리지 않았습니다.

탭이 4개→5개가 되면서 가장 좁은 iPhone(360px 폭)에서 알약 바가 화면 폭(여백 32px 뺀 328px)을 넘어설 뻔했습니다(실측 332px) — 각 탭의 좌우 여백을 16px→13px로 줄여 302px로 맞췄습니다(라벨이 전부 "오늘/책장/추가/숲길/숙제" 2글자라 줄여도 안 빡빡함, CSS 그대로 복제한 정적 HTML을 Playwright로 360~430px에서 실측 확인).

**질문에 대한 답**: 책 체크(✓)나 기록 남기기 화면의 저장은 이미 책장에 바로 꽂힙니다 — 책장은 "그 아이의 독서기록 전부"를 보여주는 화면이라, 어떤 상태(읽고 싶어요/읽는 중/다 읽음)로 기록해도 즉시 책장에 나타납니다. 책갈피(🔖) 토글은 "아직 안 읽었지만 담아두기" 전용이고, 체크는 "읽었어요"로 기록을 만들거나 상태를 바꾸는 것 — 이번에 추가한 "추가" 탭은 이 기존 흐름(책 찾기→기록 저장→책장에 반영)으로 가는 새 입구일 뿐, 별도 로직은 없습니다. DB 변경 없음.

**후속(사용자 요청: "다른 색상으로 지정해줘 연두색이나 여튼 어울리는 색으로")**: "추가" 탭만 기존 초록 포인트(`--point`)와 다른 **연두색**(`--sprout` #8BC34A / `--sprout-deep` #558B2F, `app/globals.css`에 신규 토큰)을 쓰도록 `bottom-nav.tsx`를 고쳤습니다. `PARENT_TABS`의 "추가" 항목에만 `accent: "sprout"`를 붙이고, 탭 렌더링에서 `isSprout`이면 아이콘 색을 켜짐·꺼짐 상관없이 항상 `--sprout-deep`으로, 켜졌을 때 배경도 연두 톤(`rgba(139,195,74,0.22)`)으로 바꿔서 다른 탭들과 구분되게 했습니다(다른 탭은 기존 초록 그대로).

**3D 숲에 아이의 기록 문구를 구름처럼 띄우는 아이디어에 대한 답**: 재밌는 방향이라고 생각하지만, 지금 3D 숲은 배지(나무·별·등불·새·버섯)만 그리는 "성취 전시" 공간이라 여기에 텍스트(기록 메모·질문 답변)까지 얹으면 정보가 너무 섞여 산만해질 위험이 있습니다. 또 아이 메모는 부모가 입력하는 자유 텍스트라 길이가 들쭉날쭉해서 3D 공간에서 늘 잘 읽히게 배치하기가 까다롭습니다(2D 카드 팝업과 달리 3D는 텍스트 렌더링 자체가 additional 라이브러리나 캔버스 텍스처 작업이 필요). 제안: 처음엔 3D에 손대지 않고, 이미 나무를 누르면 뜨는 안내 카드에 "그 나무(책)를 기록할 때 남긴 메모"를 한 줄 보여주는 정도로 가볍게 시작해 보고, 반응이 좋으면 그다음에 진짜 3D 구름 텍스트로 확장하는 게 안전합니다. 이번엔 구현하지 않았습니다(사용자 확인 대기).

## 나무 안내 카드에 "그 책 기록할 때 남긴 메모" 한 줄 (사용자 확인: "그래 가볍게 시작하자")

- **`lib/badge-data.ts`**: `loadForestData()`의 `reading_records` 조회에 `created_at`·`parent_memo`·`books(title, ...)`를 추가하고, `status='done'` 기록을 `read_date`(같으면 `created_at`)순으로 정렬해 **몇 번째 완독인지**를 셉니다. 권수 마일스톤(`MILESTONE_COUNTS`) 하나하나는 정확히 "그 순번째 완독"이 채운 나무이므로(예: 10그루 마일스톤 = 10번째로 다 읽은 책), `doneSorted[count - 1]`의 책 제목·`parent_memo`를 `milestoneMemos: Record<count, {title, memo}>`로 함께 돌려줍니다.
- **`forest-view.tsx`/`forest-3d-screen.tsx`**: 나무를 누르면 뜨는 안내 카드에 배지 설명 아래 한 줄을 추가해 「책 제목」과 (있으면) `· "메모"`를 보여줍니다. 별·등불·새 같은 장식(나무가 아닌 것)에는 안 뜹니다 — 장식은 책 한 권과 매핑되지 않는 배지들이라서.
- 메모가 없는 기록이면(대부분의 기존 기록) 제목만 뜹니다. DB 변경 없음(기존 컬럼만 더 읽음).

## 프로필 화면 — 아이 이름 고치기 + 아이/숲지기 구분선 + 선택된 아이 강조 (사용자 요청, 스크린샷 첨부)

- **아이 이름 고치기**: `숲지기 이름`에만 있던 "고치기" 버튼을 아이 프로필 각 줄에도 추가했습니다(`components/child-switcher.tsx`). 누르면 그 줄이 이름 입력창 + 취소/저장으로 바뀌고, `children.name`을 업데이트합니다(기존 "guardians update own children" RLS로 이미 허용됨 — 새 정책 필요 없음). 저장하면 활성 아이 이름이 바뀐 경우를 위해 기존 "chaeksup:profile-changed" 이벤트를 쏴서 상단 "OO의 책숲" 제목도 즉시 갱신되게 했습니다.
- **아이 ↔ 숲지기 사이 구분선**: `app/more/page.tsx`의 "숲지기 프로필" 섹션 앞에, 이미 그 위(프로필 전환 스위치 → 아이 프로필 사이)에 쓰던 것과 같은 옅은 구분선(`rgba(38,54,43,0.08)`, `border-top`)을 추가했습니다.
- **선택된 아이 강조**: 숲지기 프로필이 이름을 상단에 늘 보여주는 것과 짝을 맞춰, 아이 목록 위에도 "선택된 아이 · {이름}"을 추가했습니다. 이름 글자에 파란 형광펜으로 밑줄을 그은 듯한 효과(`background: linear-gradient(180deg, transparent 55%, rgba(58,134,255,0.38) 55%)`)를 줘서 눈에 잘 띄게 했습니다.
- DB 스키마 변경 없음(children.name 업데이트는 기존 컬럼).

## "추가" 탭을 항상 연두 알약으로 (사용자 요청: "누르면 더 진해져야겠지만")

다른 탭은 그 화면에 있을 때만 배경 알약 + 이름이 펼쳐지는데, "추가"는 스크린샷에서 항상 알약 상태로 보이길 원해서 규칙을 하나 더 뒀습니다 — `bottom-nav.tsx`에서 `isSprout` 탭은 **항상** 연두 배경(`rgba(139,195,74,0.22)`)과 "추가" 글자를 보여주고, 그 화면(`/library/add`)에 실제로 있을 때만 배경이 더 진해집니다(`rgba(139,195,74,0.42)`). 라벨이 상시 하나 더 펼쳐지면서 알약 전체 폭이 늘어나, 다른 활성 탭 라벨과 "추가" 라벨이 동시에 펼쳐진 최악의 경우를 360px 폭에서 다시 실측(정적 HTML + Playwright)해 328px 예산을 6px 넘는 걸 확인하고, 탭 좌우 여백을 13px→12px로 한 단계 더 줄여 324px로 맞췄습니다.

## ISBN으로 안 찾아지는 책(전권 세트 등)은 표지를 직접 찍어 올리기 (사용자 요청)

카카오 책 검색은 전권 세트("총N종")처럼 낱권에 ISBN이 안 붙은 책은 애초에 검색·바코드 조회 자체가 안 되고(위 "책 검색에서 '과학공룡'…" 항목의 한계), 이런 책은 직접 입력 경로로 등록해도 표지가 없어 책장에서 밋밋한 회색 칸으로만 보였습니다.

- **마이그레이션 0025**: 공개 Storage 버킷 `book-covers`를 추가했습니다. `reading-media`(아이별 비공개, 서명 URL)와 달리 책 표지는 `books`/`book_isbns`처럼 로그인한 사용자 전체가 보는 공유 카탈로그 정보라, 공개 버킷으로 만들어 서명 없이 기존 `books.cover_url` 자리에 공개 URL을 그대로 넣고 다른 화면들(책장 선반·숲길·숙제 목록 등)이 손댈 필요 없이 바로 보여줄 수 있게 했습니다. 경로는 `{업로더 user_id}/{uuid}.{ext}` — 소유권 판정 기준이 child_id가 아니라 "누가 올렸는가"뿐이라 이걸로 충분합니다. **SQL Editor에서 실행 필요.**
- **`lib/storage.ts`의 `uploadBookCover()`**: 파일을 업로드하고 `getPublicUrl()`로 공개 URL을 바로 돌려줍니다(서명 불필요).
...
## 숙제 탭 마감일 그룹 후속 — 머리글 들여쓰기 정렬 + 중복 마감 날짜 제거 (사용자 스크린샷 지적)

- **들여쓰기 정렬**: 방금 마감일로 바꾼 `LogGroup` 머리글("9/14까지")이 그 아래 `LogRow`의 날짜 칸("9/11")보다 살짝 더 오른쪽에서 시작해 어긋나 보였습니다 — `LogGroup`은 `px-5`(22.5px), `LogRow`는 `px-4`(18px)로 서로 다른 좌우 여백을 쓰고 있던 게 원인이었습니다. `components/log-row.tsx`의 `LogGroup` 머리글 여백을 `LogRow`와 같은 `px-4`로 맞췄습니다 — `LogGroup`을 쓰는 다른 화면(숲지기 추천도서·숙제 목록 등)에도 같이 적용돼 전반적으로 더 가지런해집니다.
- **"~9/14" 중복 제거**: "9/11 숙제가 9/14까지인 건데 ~9/14가 또 필요하냐"는 지적이 정확했습니다 — 마감일로 묶은 섹션 안의 숙제는 전부 그 섹션의 마감일과 정확히 같으므로(그룹핑 기준 자체가 마감일이라), 각 줄의 작은 날짜 칸에 있던 `~9/14`(dateBottom)는 머리글과 100% 같은 값의 중복이었습니다. `components/assignment-today.tsx`에서 `LogRow`에 `dateBottom`을 더 이상 넘기지 않고 시작일("9/11")만 보여줍니다. 숲지기용 그룹명 기준 목록(`app/teacher/assignments/page.tsx`)은 마감일이 헤딩에 없으므로 `~마감일` 표시를 그대로 남겼습니다.
- DB 변경 없음. build+lint로 확인했습니다.

## 숲지기 추천도서 탭에 책 추가 버튼이 안 보이던 문제 (사용자 질문: "추천도서 추가는 어떻게하지")

숲지기 추천도서 탭(`/teacher/books`)의 그룹 카드는 미리보기 모드(`ManagedLogList`의 `preview` prop)라 머리글 오른쪽에 **"관리 ›"뿐**이고, 실제 "+ 책 추가" 링크는 그 안의 그룹별 관리 화면(`/teacher/books/manage?group=`)에만 있었습니다 — "관리"를 눌러야 추가할 수 있다는 게 화면만 봐서는 전혀 안 드러나는 구조였습니다(숙제 탭은 미리보기 모드가 아니라 "+ 숙제 만들기"가 바로 보여서 같은 문제가 없었습니다). `components/managed-log-list.tsx`의 미리보기 머리글에 **"+ 책 추가 · 관리 ›"**를 나란히 보여주도록 고쳐서, 추천도서 탭에서 바로 추가 화면(`/teacher/books/add?group=`)으로 갈 수 있습니다. DB 변경 없음. build+lint로 확인했습니다.

## 숲지기 대시보드 숙제 목록 마감일순 정리 + "+ 새 그룹"/"+ 숙제 만들기" 이름 불일치 정리 (사용자 스크린샷 피드백)

- **대시보드 그룹 카드의 숙제 목록이 무한정 늘어날 수 있었던 문제**: `app/teacher/page.tsx`의 그룹 카드 안 숙제 목록(`assignmentProgress`)이 정렬도 없이 그룹의 숙제 전부를 그대로 나열하고 있어서, "몇 개까지 나타나게 할 거냐"는 지적이 정확했습니다. 아이 쪽 숙제 탭(`assignment-today.tsx`)이 이미 쓰던 `effectiveRange()`(`lib/assignment-period.ts`)로 마감일을 계산해 **마감이 이른 순으로 정렬**하고, 그룹당 **최대 3개**(`ASSIGNMENT_PREVIEW_LIMIT`)만 보여준 뒤 넘는 만큼은 "나머지 N개 · 전체 보기 ›"(`/teacher/assignments`)로 이어지게 했습니다. 각 줄에도 "9/14까지"처럼 마감일을 붙여서(요청한 "언제까지인지에 따라 구분") 어떤 숙제가 급한지 바로 보이게 했습니다. 이걸 위해 `operatorGroupsQuery()`의 `assignments` 임베드에 `start_date, end_date, created_at`을 추가했습니다.
- **"+ 새 그룹"(추천도서 탭) vs "+ 숙제 만들기"(숙제 탭) 이름 불일치**: 추천도서 탭(`/teacher/books`) 맨 위 버튼이 "+ 새 그룹"으로 `/recommend/create`(그룹 자체를 새로 만드는 화면)로 갔는데, 숙제 탭은 "+ 숙제 만들기"로 그 그룹 안에 숙제 하나를 추가하는 화면(`/teacher/assignments/new`)으로 갔습니다 — 같은 자리의 버튼인데 하나는 "그룹 만들기", 하나는 "콘텐츠 추가"라 이상하다는 지적이었습니다. 단순히 라벨만 "+ 추천도서 만들기"로 바꾸면 실제로는 여전히 새 그룹을 만들어버려 라벨과 동작이 어긋나므로, **링크 자체를 `/teacher/books/add`로 바꿔** 숙제 탭과 완전히 같은 패턴(그룹이 여럿이면 고르기 → 하나면 바로 폼)으로 맞췄습니다. 진짜 "새 그룹 만들기" 기능은 이미 대시보드(`/teacher`) 그룹 카드 아래의 점선 "+ 새 그룹 만들기" 버튼으로 옮겨져 있던 상태라(이전 라운드), 추천도서 탭에서 빼도 여전히 대시보드에서 만들 수 있습니다.
- DB 변경 없음. build+lint로 확인했습니다.

## 숲지기 하단 탭 이름 항상 표시 + 숙제 탭도 마감일 기준으로 정리 (사용자 요청: "누를때만 메뉴 이름 나오는 게 아니라 항상 나오게", "숲지기한테도 숙제는 언제까지인지가 중요")

- **숲지기 알약 바 — 아이콘 4개만으론 헷갈린다는 지적**: 부모 탭(오늘·책장·추가·숲길·숙제)은 5개라 켜진 탭만 이름이 펼쳐지는 게 자연스러웠지만, 숲지기 탭(대시보드·아이들·추천도서·숙제)은 4개뿐인데도 아이콘만 보고는 헷갈린다는 스크린샷 지적을 받았습니다. `components/bottom-nav.tsx`에 `alwaysShowLabels = role === "operator"`를 추가해, 숲지기일 때는 켜짐과 무관하게 라벨을 항상 펼칩니다. 4칸을 전부 펼치면 가장 좁은 iPhone(360px)에서 알약이 넘칠 위험이 있어(정적 HTML 복제본을 Playwright로 실측 — 기존 패딩·간격 그대로면 340px로 328px 예산을 넘김), 숲지기 탭에 한해 칸 좌우 여백을 12px→6px, 아이콘-글자 간격을 6px→2px, 글자 크기를 13px→12px로 줄여 최대 폭이 326px(360px 기준 예산 328px 이내)이 되도록 맞췄습니다. 부모 탭(추가 탭 포함)의 기존 패딩·크기는 그대로입니다.
- **숲지기 숙제 탭도 마감일 기준으로 재정리**: "숲지기한테도 숙제는 언제까지 숙제인지가 중요. 그 기준으로 정리"라는 요청으로, `app/teacher/assignments/page.tsx`의 묶음 기준을 그룹명에서 마감일(`effectiveRange(card).end`)로 바꿨습니다 — 아이 쪽 숙제 탭(`components/assignment-today.tsx`)에서 이미 쓰던 것과 정확히 같은 규칙입니다. 머리글은 "9/14까지"처럼 마감일, 그 밑 작은 글자(headingSub)에 그 마감일에 걸린 그룹 이름들("7살-추천도서 · 6세-추천도서")과 숙제 개수를 적어 여러 그룹 숙제가 같은 마감일에 섞여도 어디 숙제인지 계속 보이게 했습니다. 마감이 이른 순 정렬. 각 줄의 작은 날짜 칸은 시작일만 남기고(`~마감일`은 머리글과 중복이라 아이 쪽에서 이미 뺀 것과 같은 이유로 제거), "+ 숙제 만들기" 링크는 이제 특정 그룹을 미리 고를 수 없어(한 마감일 섹션에 여러 그룹이 섞일 수 있음) 그룹 없이 `/teacher/assignments/new`로 보내 그 화면의 "어느 그룹에?" 선택 단계를 그대로 타게 했습니다. 그룹이 있는데 아직 낸 숙제가 하나도 없으면(예전엔 그룹마다 빈 카드가 보였음) 이제 "아직 낸 숙제가 없어요." 안내 한 줄만 뜨고, 상단의 "+ 숙제 만들기" 버튼으로 그룹을 골라 처음 만들 수 있습니다.
- DB 변경 없음. build+lint로 확인했습니다.

## 숙제·추천도서를 여러 그룹에 한 번에 (사용자 요청: "그룹 모두에 추천도서와 숙제 동시에 넣을 수도 있단다. 그룹 선택버튼 넣어서 여러그룹에 동시에 올릴 수 있도록")

- **`app/teacher/assignments/page.tsx`의 그룹별 "+ 숙제 만들기" 줄 버튼이 두 줄로 꺾이던 문제**: 마감일 섹션 머리글 오른쪽이 이미 날짜·그룹명·숙제 개수로 붐비는 자리라 "+ 숙제 만들기"(7글자)가 공간이 모자라 줄바꿈됐습니다(스크린샷). 그룹별 줄 버튼만 **"+ 숙제"**로 줄였습니다(페이지 상단의 큰 버튼은 자리가 넉넉해 "+ 숙제 만들기" 그대로).
- **`components/operator-group-multi-picker.tsx`(신규)**: 기존 `OperatorGroupPicker`(그룹을 누르면 그 그룹으로 바로 이동 — "관리" 화면처럼 한 그룹만 볼 때 그대로 유지)와 별개로, **체크박스로 여러 그룹을 동시에 고르는** 새 컴포넌트를 만들었습니다. 고른 그룹 id들을 콤마로 이어 `?groups=id1,id2`로 넘기고, 아래 "선택한 N개 그룹에 숙제 내기/올리기" 버튼을 누르면 이동합니다.
...
## 숲지기 대시보드 재정의 — 그룹 카드 나열 → 알림·시각화 화면 (사용자 지적: "대시보드의 역할 모르겠어, 관리를 들어가도 추가 기능이 또 있다")

숲지기 4탭 중 아이들·추천도서·숙제는 각각 하나의 대상(사람/책/할일)을 관리하는 역할이 뚜렷한데, 대시보드는 그룹마다 카드를 나열하고 각 카드 안에 숙제 진행 현황(추천도서·숙제 탭과 내용 중복) + "관리하기"(누르면 그룹 상세 화면인데, 거기 또 추천도서 올리기·숙제 만들기 폼이 있어 추가 기능 진입점이 3중으로 겹침) 구조라 존재 이유가 애매했습니다. `AskUserQuestion` 없이 방향(알림·요약 중심으로 좁히고 시각화 추가)에 동의를 받아 다음과 같이 재정의했습니다.

- **`app/teacher/page.tsx` 전면 재작성**: 그룹별 카드 나열을 없애고 세 블록으로 재구성했습니다.
  1. **한눈에 보는 숫자**(그룹/아이/진행 중 숙제/승인 대기 4칸 — `Stat` 컴포넌트, 오늘 탭 요약 박스와 같은 위계). "아이" 수는 그룹별 멤버 수를 단순 합산하지 않고 `child_id` 기준으로 전체 그룹을 가로질러 **중복 제거**해서 셉니다(한 아이가 여러 그룹에 속해도 한 명으로).
  2. **승인 대기**(있을 때만 뜨는 `Section`): 그룹별로 대기 인원과 "확인 ›" 링크만 — 실제 승인/거절 액션은 그룹 상세(`GroupApprovals`)에서 그대로 합니다.
  3. **마감 임박 숙제 시각화**: 전체 그룹을 가로질러 완료되지 않은 숙제를 마감일순으로 최대 5개 뽑아, 완료 인원 비율만큼 **막대그래프**(`--point` 채움, 마감이 지났으면 `--berry`)로 보여줍니다. 이게 이번에 요청받은 "시각화"이고, 숙제 탭의 전체 목록과 겹치지 않도록 "지금 급한 것"만 추리고 "전체 보기 ›"로 숙제 탭을 가리킵니다.
  4. **운영 중인 그룹**(flush `Section`, 구분선 목록): 그룹 이름·유형·아이 수만 보여주고, 오른쪽엔 "관리하기"가 아니라 **"설정 ›"**(그룹 상세로) — 그룹 자체(이름·소개·삭제)를 만지는 자리와 콘텐츠(책·숙제)를 채우는 자리가 다르다는 걸 라벨로도 구분했습니다.
...
- **`components/group-top-select.tsx`(신규)**: 제목 옆에 놓는 작은 드롭다운. `?group=` 쿼리로 그룹을 고르면 그 페이지가 그 그룹 하나만 다시 그립니다. **그룹이 하나뿐이면 호출부가 아예 렌더링하지 않습니다** — 고를 게 없는데 드롭다운이 떠 있는 것 자체가 "그룹"이라는 개념을 억지로 노출하는 것이라, 대다수인 단일 그룹 숲지기에게는 이 컴포넌트 자체가 안 보입니다.
- **세 탭 전부 "그룹 하나만 보여주는 화면"으로 재구성**: `app/teacher/children/page.tsx`, `app/teacher/books/page.tsx`, `app/teacher/assignments/page.tsx`가 이제 `?group=`(없으면 첫 그룹)으로 정한 그룹 하나의 내용만 그립니다. 그룹별로 세로로 쌓아 보여주던 기존 구조(그룹마다 헤딩 + "그룹 관리"/"관리" 링크 반복)를 없앴습니다.
- **추천도서 탭이 곧 관리 화면이 됨**: 예전엔 탭에 그룹마다 미리보기 10권만 뜨고 "관리 ›"를 눌러야 진짜 화면(`/teacher/books/manage?group=`)으로 넘어갔는데, 이제 탭 자체가 선택된 그룹의 전체 목록 + 추가·선택·삭제를 바로 보여줍니다. 별도 관리 화면이 필요 없어져 **`/teacher/books/manage`를 완전히 지웠습니다**(`components/operator-group-picker.tsx`도 그 화면에서만 쓰여서 함께 삭제, `ManagedLogList`의 `preview` 모드도 안 쓰여서 걷어냈습니다). 그룹 상세 화면(`/recommend/[groupId]`)의 "추천도서 관리 ›" 링크도 `/teacher/books?group=`으로 바꿨습니다.
- **숙제 탭도 그룹 하나로 좁힘**: 예전엔 여러 그룹의 숙제를 마감일로 한데 묶어 헤딩마다 그룹 이름들을 나열했는데(`headingSub`에 "7살-추천도서 · 6세-추천도서"), 이제 그룹을 먼저 고르므로 그 나열이 필요 없어져 뺐습니다. "+ 숙제 만들기" 링크도 이제 선택된 그룹으로 바로 이어집니다(`?group=` 자동 포함) — 그룹이 하나면 "어느 그룹에?" 선택 화면 자체를 안 거칩니다.
- **여러 그룹에 동시에 내는 기능은 남겨둠**: 직전 라운드에서 만든 "여러 그룹 동시에 숙제/추천도서 올리기"(`OperatorGroupMultiPicker`)는 여전히 필요한 기능이라(사용자가 명시적으로 요청했던 것) 지우지 않았습니다. 대신 그룹이 둘 이상일 때만 "여러 그룹에 함께 내려면/올리려면 ›"라는 작은 보조 링크로 한 단계 뒤로 물러났습니다 — 기본 흐름(그룹 하나 선택 → 그 그룹에 바로 추가)을 가리지 않으면서, 필요한 사람은 여전히 쓸 수 있습니다.
- **그룹 설정(이름·소개·삭제·승인)은 대시보드에서만**: 세 탭 어디에도 "그룹 관리"/"그룹 설정" 링크를 두지 않았습니다 — 그룹 자체를 만지는 진입점은 대시보드의 "운영 중인 그룹" 목록(지난 라운드에서 만든 "설정 ›") 하나로 통일해서, "관리"라는 말이 여러 화면에 흩어져 있던 문제를 마저 정리했습니다.
- **대시보드의 "마감 임박 숙제" 항목을 개별 링크로**: 숙제 탭이 그룹 하나만 보여주게 되면서, 대시보드에서 다른 그룹의 급한 숙제를 눌러 "전체 보기"로 가면 엉뚱한 그룹이 열리는 문제가 생길 뻔했습니다. 각 줄을 그 숙제의 상세 화면(`/teacher/assignments/{id}`, 그룹과 무관하게 항상 맞는 곳으로 감)으로 바로 연결해서 피했습니다.
- **상세 화면들의 "← 뒤로" 링크도 그룹을 기억하도록 수정**: 책 상세(`/teacher/books/[bookId]`)·아이 상세(`/teacher/children/[childId]`)·숙제 상세(`/teacher/assignments/[assignmentId]`)·숙제 만들기·책 올리기 화면의 뒤로가기 링크가 전부 `?group=`을 그대로 들고 돌아가도록 고쳤습니다 — 안 그러면 그룹이 여럿인 숲지기가 두 번째 그룹의 상세를 보다가 뒤로 가면 첫 번째 그룹으로 튕기는 문제가 생깁니다.
- 정적 HTML로 360px 폭에서 그룹 드롭다운이 제목과 함께 자연스럽게 들어가는지 Playwright로 확인했고, DB 변경 없어 build+lint로 검증했습니다.

## 아이들·추천도서·숙제의 그룹 선택을 하나로 공유 (사용자 지적: "그룹을 더 상위에서 바꾸고 하위메뉴가 다 전환되게, 매 메뉴마다 설정하면 귀찮잖아")

방금 라운드에서 세 탭에 그룹 드롭다운을 각각 넣었는데, 각 드롭다운이 그 탭만의 URL 쿼리(`?group=`)로 따로 동작해서 — 아이들 탭에서 "7살 반"을 골라도 추천도서 탭으로 넘어가면 다시 첫 그룹으로 돌아가 있었습니다. 사용자가 원한 건 "그룹은 한 번만 고르고, 세 탭이 전부 그 선택을 공유"하는 것이었습니다.

- **마이그레이션 0026**: `users.active_operator_group_id`(부모 쪽 `active_child_id`와 같은 패턴)를 추가하고, self-update RLS 정책에 "이 값이 있으면 반드시 내가 승인된 운영진으로 있는 그룹이어야 한다"는 검사를 더했습니다. **이 세션엔 드물게 로컬 Postgres 클러스터가 실제로 떠 있어서**(`pg_ctlcluster 16 main start`로 기동, 이전 세션들이 만들어 둔 mock `auth` 스키마·`authenticated`/`tester` 롤이 그대로 남아 있었음), 이번엔 문법 확인에 그치지 않고 실제 공격 시나리오까지 검증했습니다 — 로컬 테스트 DB(그동안 쓰이지 않아 마이그레이션 0019~0025가 밀려 있었던 것도 먼저 따라잡았습니다)에 그룹 2개 중 하나만 운영하는 사용자를 만들어, (1) 자기가 운영하는 그룹으로 설정 → 성공, (2) 운영하지 않는 다른 그룹으로 설정 시도 → RLS가 실제로 막음(`new row violates row-level security policy`), (3) `null`로 되돌리기 → 성공, 셋 다 확인했습니다. **SQL Editor에서 실행 필요**(실제 Supabase 프로젝트는 이 세션이 직접 못 건드림).
- **`lib/active-operator-group.ts`의 `pickActiveGroupId()`**: 화면에 보여줄 그룹을 정하는 우선순위 — URL의 `?group=`(다른 화면에서 특정 그룹으로 들어온 경우, 예: 책 상세의 "← 추천도서") > `users.active_operator_group_id`(그룹 드롭다운이 저장해 둔 값) > 첫 그룹. 세 탭(`app/teacher/children`, `app/teacher/books`, `app/teacher/assignments`) 전부 이 함수 하나로 그룹을 정합니다.
- **`components/group-top-select.tsx`**: 드롭다운을 바꾸면 화면은 바로 그 그룹으로 이동하고(URL 갱신), 동시에 `users.active_operator_group_id`를 그 값으로 업데이트합니다. 그래서 아이들 탭에서 그룹을 바꾸고 하단 탭으로 추천도서·숙제 탭에 가면(둘 다 URL에 `?group=`이 없는 평범한 이동), DB에 저장된 값을 읽어 **같은 그룹**이 그대로 보입니다 — "매 메뉴마다 다시 골라야 하는" 번거로움이 없어졌습니다.
- **기존 "← 뒤로가기" 링크들의 `?group=`은 그대로 유지**: 지난 라운드에서 책/아이/숙제 상세 화면과 만들기 화면에 붙여둔 `?group=` 쿼리는 안 지웠습니다 — `pickActiveGroupId()`가 URL 쿼리를 최우선으로 보므로, "이 특정 항목이 속한 그룹"으로 정확히 돌아가는 동작은 그대로 유지하면서도, 그게 "앞으로의 기본 그룹"을 영구히 바꾸는 건 아니게(그 링크들은 DB를 쓰지 않으므로) 구분됩니다 — 대시보드의 "마감 임박 숙제"를 눌러 다른 그룹의 숙제 상세를 잠깐 보는 것과, 드롭다운으로 "이제부터 이 그룹을 기본으로 보겠다"고 정하는 것은 서로 다른 의도라 이렇게 나눴습니다.

## 숲지기 대시보드 스탯 4칸 — 클릭 가능하게 + 기준 각주 + 간격 균등화 (사용자가 스크린샷에 손그림으로 표시)

대시보드 상단 4칸(그룹/아이/진행 중 숙제/승인 대기)을 손그림으로 짚으며 네 가지를 요청했습니다: (1) 아이 수가 한 아이가 여러 그룹에 속하면 몇 명으로 세는지 기준을 작게 표시, (2) "진행 중 숙제"를 "오늘 숙제"로 바꾸고 오늘까지 해야 하는 숙제 개수로, (3) 네 칸 간격을 균등하게, (4) 네 칸 모두 각각의 의미에 맞는 화면/섹션으로 클릭 이동.

- **아이 수 집계 기준 각주**: 이미 `child_id` 기준 `Set`으로 중복 없이 세고 있었지만(여러 그룹에 속해도 한 명) 화면에 그 기준이 안 보였습니다. "아이" 라벨에 `*`를 붙이고 스탯 박스 아래에 작은 각주(`* 여러 그룹에 속해도 아이는 한 명으로 세어요`, 10px)를 달았습니다. 처음엔 칸 안에 바로 여러 줄로 넣어봤는데(4분할 칸 하나가 70px 안팎이라) 좁은 칸 안에서 "여러 그룹 소속도 한 명으로 세요"가 세 줄로 어색하게 꺾여서, 각주를 칸 밖 전체 너비로 빼는 방식으로 바꿨습니다(Playwright 360px 렌더로 확인).
- **"오늘 숙제"**: `urgent`(완료 안 된 숙제) 중 마감일이 오늘이거나 이미 지난 것의 개수(`dueTodayCount`)로 바꿨습니다. "진행 중"(마감이 안 지난 전체)보다 좁혀서, 여유 있는 숙제까지 다급해 보이지 않게 했습니다. 마감 임박 숙제 섹션의 "완료됨은 건너뛴다"는 기존 규칙은 그대로 재사용.
- **간격 균등화**: `flex items-center justify-between`을 `grid grid-cols-4 gap-2`로 바꿔, 칸 너비가 내용 길이와 무관하게 항상 4등분으로 고정되고 칸 사이 간격도 항상 같습니다.
- **클릭 이동**: `Stat` 컴포넌트에 `href` prop을 추가해 각 칸이 `<Link>`가 되도록 했습니다 — 그룹 칸은 같은 페이지 안의 "운영 중인 그룹" 섹션으로(`#operating-groups` 앵커, `components/section.tsx`에 `id` prop을 추가해 앵커 타깃으로 씀), 아이 칸은 `/teacher/children`, 오늘 숙제 칸은 `/teacher/assignments`로, 승인 대기 칸은 대기 인원이 있을 때만(`totalPending > 0`) 같은 페이지의 "승인 대기" 섹션(`#pending-approvals`)으로 — 대기 인원이 0명이면 누를 곳이 없으므로 링크를 아예 안 답니다. 서버 컴포넌트라 자바스크립트 없이 브라우저 기본 앵커 스크롤로 동작합니다.
- Playwright 360px 정적 렌더로 각주 줄바꿈과 4칸 간격을 확인했고, DB 변경이 없어 build+lint로 검증했습니다.

## 숲지기 그룹 전환 바를 고정 상단으로 (사용자 요청: "그룹 전환은 고정상단이길 바라는거야. 흰색 배경 박스로 해서")

박스 타일 자체는 마음에 들어했지만, 페이지 본문과 함께 스크롤되어 사라지는 게 아니라 상단바(`TopBar`)처럼 **항상 화면에 붙어 있는 흰색 바**를 원한 것으로 확인됐습니다(스크린샷의 손그림이 상단바와 타일 줄을 하나로 묶어 표시). 겸사겸사 "대시보드는 이 모든 걸 통합하니까 메뉴를 좀 별도로 하라는 소리"라고 이전 요청의 의도를 다시 확인해 주셨는데 — 이건 이미 구현대로입니다(대시보드는 그룹을 가로지르는 요약 화면이라 그룹 전환 바가 없고, 아이들/추천도서/숙제 세 화면만 그룹 하나를 골라 보는 구조라 전환 바가 있음).
...
## 승인 대기 스탯 클릭 시 앵커 스크롤이 상단바에 가려 보이던 것 (사용자 지적: "이상한 위치로 이동해")

대시보드의 "승인 대기" 스탯을 눌러 `#pending-approvals` 앵커로 스크롤하면, 브라우저 기본 동작은 그 섹션의 맨 위를 뷰포트 맨 위(y=0)에 딱 맞춥니다. 그런데 화면 맨 위엔 항상 `TopBar`(52px 높이 고정)가 떠 있어서, 정작 스크롤해 간 섹션의 제목("승인 대기")이 그 밑에 가려 안 보이고 설명 문구("1명이 기다리고 있어요")만 뎅그러니 화면 위에 걸쳐 보였습니다(스크린샷) — "이상한 위치로 이동했다"는 게 정확한 증상이었습니다. `components/section.tsx`에 `id`가 있는 섹션에만 `scroll-margin-top: 68px`(상단바 52px + 여유 16px)을 줘서, 앵커 스크롤이 상단바 바로 아래에서 멈추도록 고쳤습니다 — 대시보드의 "그룹"(`#operating-groups`)·"승인 대기"(`#pending-approvals`) 스탯 클릭 둘 다 해당됩니다. DB 변경 없음. build+lint 통과.

## 대시보드 "아이" 스탯 눌러 이동하면 이전 스크롤 위치가 남아 있던 것 (사용자 지적: "상단바와 아이 목록 겹쳐보이는데")

대시보드를 스크롤해 내린 상태에서 "아이" 스탯(→ `/teacher/children`, 앵커가 아니라 실제 다른 라우트로 이동)을 누르면, 새로 연 아이들 화면이 훨씬 짧은데도 스크롤이 맨 위로 안 돌아가고 이전 스크롤 위치가 그대로 남아 있었습니다 — 그래서 화면 중간(짧은 목록의 끝자락)에서 시작된 것처럼 보이고, 그 아래는 빈 배경만 남습니다. `/teacher`와 `/teacher/children` 둘 다 `loading.tsx`가 있는(서버에서 데이터를 불러오는 동안 스트리밍되는) 라우트라, Next.js가 화면 전환 시 자동으로 맨 위로 스크롤해 주는 동작이 이 스트리밍 타이밍과 어긋나 스크롤이 안 돌아오는 경우가 있는 걸로 보입니다(같은 원인이 `/teacher/books`·`/teacher/assignments`로 이동할 때도 잠재적으로 있을 수 있음).
...
## "안보여" — getActiveChild()가 조회 실패를 빈 상태로 위장 (사용자 신고)

'그룹' 탭을 새로 합친 직후, 그 화면 본문이 "더보기에서 아이를 추가해 주세요"만 보여준다는 신고("안보여")를 받았습니다. 원인은 두 항목 전(부모 쪽 그룹 전환 공유)에서 `getActiveChild()`의 두 select에 새 컬럼 `active_group_id`를 추가했는데, 그 컬럼을 만드는 마이그레이션 0027이 실제 Supabase 프로젝트에 아직 적용되지 않은 상태로 배포됐기 때문이었습니다. 상단의 `ChildGroupBar`(클라이언트에서 직접 쿼리)는 `active_group_id`를 안 쓰는 다른 select라 정상적으로 그룹 타일을 보여주고 있어서, 서버 쪽 `getActiveChild()`만 조용히 실패하고 있다는 게 단서였습니다 — 컬럼이 없으면 select 자체가 에러를 반환하는데, 두 조회 모두 에러를 확인하지 않고 `data`만 봐서 "쿼리 실패"와 "정말 아이가 없음"을 구분하지 못하고 있었습니다(CLAUDE.md의 "전체 점검 라운드"에서 이미 한 번 잡았던 것과 같은 클래스의 버그가 이번 컬럼 추가로 재발).

- **`lib/active-child.ts`**: 두 select 모두 `error`를 확인해서, 에러가 있으면 조용히 `null`로 넘어가지 않고 그대로 던집니다(`throw`) — 이 함수를 부르는 8개 화면 전부의 시그니처를 바꾸는 대신, 이미 있는 `app/error.tsx`(전역 에러 바운더리, "길을 잠깐 잃었어요" + 다시 시도)가 잡도록 했습니다. `getActiveChild()`는 거의 모든 부모 화면이 부르는 핵심 함수라, "빈 상태로 위장"보다는 "확실한 오류 화면 + 재시도 버튼"이 원인 파악에 훨씬 낫습니다.
- **근본 조치**: 마이그레이션 0027을 아직 못 돌렸다면 지금 이 문제가 그대로 재현됩니다 — **SQL Editor에서 0027을 실행해야** `active_group_id` 컬럼이 생기고 정상으로 돌아옵니다. build+lint로 확인했습니다(로컬 Postgres 없이 순수 애플리케이션 코드 변경).

## "그룹 탭에 추천도서/숙제 구분이 안 보여" — 본문이 고정 그룹 바 뒤에 숨어 있었음 (사용자 신고, 시크릿 모드에서도 재현)

마이그레이션 0027을 돌린 뒤에도 '그룹' 탭에 "추천도서/숙제" 알약 탭과 "모든 그룹의 추천도서" 제목이 안 보인다는 신고를 받았습니다. 배포 커밋 일치·빌드 캐시 없이 재배포·시크릿 모드까지 전부 확인해도 그대로였고, 임시 미리보기 라우트로 같은 JSX를 렌더링하면 정상이라 한참 헤맸습니다. 스크린샷의 픽셀 위치를 재보니 분야 칩이 그룹 바 바닥에서 5px 아래에서 시작하고 있었습니다 — 즉 알약 탭·제목은 **렌더링은 됐지만 고정 그룹 바 뒤에 가려져** 있었고, 사용자가 "상단 고정 메뉴 뒤에 숨었나?"라고 짚은 게 정답이었습니다.

- **원인**: `CHILD_GROUP_BAR_HEIGHT`·`OPERATOR_GROUP_BAR_HEIGHT`를 `"use client"` 파일(`child-group-bar.tsx`·`operator-group-bar.tsx`)에서 export하고, 서버 컴포넌트 페이지가 그걸 import해서 `paddingTop: \`${32 + H}px\``로 썼습니다. React 서버 컴포넌트에서 `"use client"` 모듈의 **모든 export는 서버 쪽에선 숫자가 아니라 클라이언트 참조 프록시**로 넘어옵니다. 실제로 서버가 내보낸 HTML은 `style="padding-top:32function() {…"`였고(임시 프로브 페이지로 확인), 브라우저는 이 잘못된 값을 버려 padding이 0이 됐습니다. 그래서 본문 첫 105px(알약 탭 + 제목)이 정확히 고정 바(높이 ~100px) 아래에 깔렸습니다. TypeScript는 `number`로 보고 빌드도 통과해서 어디서도 안 잡혔습니다. 로컬 미리보기에서 정상이었던 건 상수를 직접 하드코딩해 재현했기 때문입니다.
- **같은 버그가 숲지기 세 탭에도**: 아이들·추천도서·숙제 탭도 같은 방식으로 `OPERATOR_GROUP_BAR_HEIGHT`를 import하고 있어, 그룹이 둘 이상이면 본문 상단이 바에 가려졌습니다. 앞서 "대시보드 '아이' 스탯 눌러 이동하면 겹쳐 보인다"를 스크롤 위치 문제로 진단하고 `ScrollToTop`을 넣었는데, 실제 원인은 이쪽이었을 가능성이 큽니다(`ScrollToTop`은 무해하니 그대로 둠).
- **수정**: 두 상수를 `"use client"`가 아닌 `lib/group-bar-height.ts`로 옮기고, 페이지 4곳이 거기서 import합니다. 값도 실제 아이폰 스크린샷에서 잰 높이(아이 바 ~101px, 글자 크기 112.5% 반영)보다 넉넉하게 아이 102 / 숲지기 82로 올렸습니다.
- **규칙**: 서버 컴포넌트가 쓸 상수·타입 외 값은 절대 `"use client"` 파일에서 export하지 않는다. 컴포넌트 파일에 상수를 두고 싶으면 별도 plain 모듈로 뺀다.

## 그룹 탭은 숙제부터 + 없어진 메뉴 이름 정리 (사용자 요청: "숙제를 먼저 보이게 하자. 그외에도 전체적으로 메뉴 싹 다 정리")

- **기본 탭 = 숙제**: `app/group/page.tsx`의 기본값을 `books` → `assignments`로 바꾸고 알약 순서도 **숙제 · 추천도서**로 뒤집었습니다 — 그룹 탭에 들어오는 가장 흔한 이유가 "오늘 뭐 해야 하지"라서 할 일을 먼저 보여주고, 둘러보기(추천도서)는 한 번 더 눌러 봅니다. 오늘 탭의 "전체 보기"와 `/assignments` 리다이렉트도 이제 `?tab=assignments`를 안 붙이고 `/group`으로 바로 갑니다(기본값이라 중복).
- **지난 숙제를 `/group` 아래로**: `app/assignments/past` → **`app/group/past`**. `/assignments`는 이미 리다이렉트만 남은 껍데기인데 그 아래에 진짜 화면 하나가 매달려 있어서, 그룹에 관한 화면은 전부 `/group` 아래 한곳에 모았습니다(예전 경로는 리다이렉트로 남김). 뒤로가기 링크도 `/group`으로.
- **화면에 남아 있던 "없는 메뉴 이름" 정리**: 하단 탭에서 사라진 지 오래인 이름들이 안내 문구에 그대로 있었습니다.
  - **"더보기에서 아이를 추가해 주세요"** (오늘·책장·그룹·우리 숲 빈 화면 + 초대코드·팔로우·그룹 참가 오류 문구, 총 7곳) → **"위쪽 프로필에서"**. '더보기'는 탭이 아니라 상단바의 얼굴+이름(→ 프로필·설정)이 된 지 오래인데 문구만 안 따라왔습니다.
  - **"숲길 탭에 이 그룹이 생기고…"**(그룹 상세의 팔로우 전 안내 2곳) → **"그룹 탭에"**. 더보기의 "도서관·크리에이터의 숲길을 팔로우해요"도 "추천도서를 팔로우해요"로(숲길은 이제 메뉴 이름이 아님).
- **제목이 없거나 옛 이름이던 화면**: `/recommend`는 로그인 상태에서 **제목이 아예 없었고**(바로 "+ 그룹 만들기" 버튼부터 시작) 로그아웃 화면만 옛 이름 "추천"이었습니다 → 양쪽 다 **"그룹 찾기"**(더보기 링크·그룹 타일의 "+"가 부르는 이름과 통일), 제목과 "+ 그룹 만들기"를 한 줄에. `/more`의 로그아웃 제목도 "더보기" → **"프로필 · 설정"**(로그인 화면과 같게).
- DB 변경 없음. build+lint 통과(37 → 38 라우트, `/group/past` 추가 + `/assignments/past` 리다이렉트).

## 하단 탭 이름 상시 표시 + 떠 있던 소제목을 카드 안으로 (사용자 요청: "아이 메뉴 하단 고정 메뉴바도 그냥 글 다 보이게 해줘. 많이 심플해졌으니. 그외에 전체적으로 디자인 정리")

- **부모 하단 탭도 이름을 항상 펼친다**: 지난 라운드에 숲지기 탭만 상시 라벨(`alwaysShowLabels = role === "operator"`)이었는데, 부모 탭이 오늘·책장·추가·그룹 4개로 줄면서 같은 대우를 해도 폭이 남습니다. 이제 역할과 무관하게 라벨이 항상 보이고(켜진 탭 표시는 배경 알약 + 아이콘 굵기로만), 글자 크기는 양쪽 다 12px입니다. 한 탭당 여백만 역할별로 다릅니다 — 숲지기는 "대시보드"·"추천도서"처럼 이름이 길어 `px-[6px] gap-[2px]`, 부모는 전부 두 글자라 조금 여유 있게 `px-[10px] gap-[4px]`. **360px 폰에서 실측**(정적 HTML + Playwright): 부모 306px / 숲지기 326px로 알약 예산(화면폭-32px = 328px) 안. 실제 컴포넌트를 임시 라우트에서 아이폰 뷰포트로 찍어 두 탭 줄이 다 들어가는 것도 확인했습니다(라우트는 확인 후 삭제).
- **배경에 떠 있던 소제목을 전부 `Section` 카드 머리로**: "제목과 내용이 한 묶음 같아 보이지 않는다"는 지적으로 `components/section.tsx`를 도입했는데, 그 뒤에 만들어진 화면 여섯 곳이 여전히 옛 방식(배경 위 `p.d text-base` + 그 아래 별도 카드)이었습니다. 전부 `Section`으로 바꿨습니다 — 숲지기의 아이 상세(추천도서·숙제), 책 상세(아이들), 숙제 상세(아이별), 그룹 찾기(둘러보기), 그룹 상세의 추천도서 선반, 그룹 탭의 추천도서 선반. 카드 안 줄 목록은 `flush`로 두고 각 줄의 좌우 여백을 **`px-[24px]`(= Section 머리글과 같은 기준선)**으로 맞췄습니다(예전엔 `px-3`이라 제목보다 줄이 안쪽으로 들어가 어긋나 보였음). 앞으로도 규칙은 하나 — 카드 머리와 그 안의 줄은 같은 24px 기준선.
- **겹쳐 있던 숫자 정리**: 책 상세는 머리글("그룹명 · N/M명 읽음")과 아이들 목록이 같은 수를 두 번 보여줬습니다 → 머리글엔 그룹 이름만, 읽은 수는 "아이들" 카드 머리 오른쪽에. 추천도서 선반도 자기 안에 "N권 · 읽은 책 M권" 줄이 있어서 Section 설명에 권수를 또 쓰지 않았습니다.
- **간격 통일**: 섹션 사이는 `mt-5`(더보기의 첫 카드만 `mt-6`이었고, 숲지기 상세 화면들은 `mt-6`/`mt-8`이 섞여 있었음), 숙제 탭의 "이번 주 숙제"/"다가오는 숙제" 소제목도 같은 `mt-6`으로.
- DB 변경 없음. lint·build 통과(38 라우트).

## 내용과 메뉴가 겹쳐 보이던 것 — 하단 바 불투명 + 소제목 탭을 고정 바 안으로 (사용자 스크린샷 지적: "여기 내용이랑 메뉴랑 겹쳐 나오니 너무 복잡하지?")

스크린샷에서 떠 있는 하단 알약 바 뒤로 숙제 카드의 글자("책읽고 독서통장 쓰기")가 비쳐 보였고, 화면 위쪽은 상단바 → 그룹 타일 바 → 숙제/추천도서 알약 → 검색창까지 메뉴가 네 겹으로 쌓여 어디부터가 내용인지 경계가 없었습니다. 두 가지를 고쳤습니다.

- **하단 알약 바를 완전 불투명으로**: `rgba(255,255,255,0.96)` → `var(--card)`. 4%만 비치는 것도 글자 위에 겹치면 그대로 읽혀서 "겹쳐 나온다"로 보였습니다(블러는 이미 iOS 스크롤 버벅임 때문에 뺀 상태라 불투명이 맞습니다). 알약은 `fixed`라 스크롤 중에는 항상 내용 위를 지나가므로, 비침을 없애는 게 유일한 해결입니다(`main`의 `pb-[96px]`이 이미 있어 **끝까지 스크롤하면** 마지막 내용이 가려지진 않습니다).
- **"숙제 / 추천도서" 소제목 탭을 고정 그룹 바 안으로**: 본문 맨 위에 알약으로 떠 있던 두 탭을 `components/child-group-bar.tsx`(고정 바)의 둘째 줄로 옮겼습니다 — 이제 **고정 바 안이 전부 메뉴, 그 아래는 전부 내용**이라는 경계가 생깁니다. 소제목 탭은 클라이언트에서 그려서 누르는 즉시 강조가 바뀌고, 그룹 선택(`?group=`)은 그대로 이어집니다.
- **그룹 타일을 한 치수 줄임**: 56px → 44px 타일, 이름 11px → 10px, 타일 간격 16 → 12px. 소제목 탭 줄이 들어왔는데도 고정 바가 예전(102px)보다 조금 커지는 선(**실측 124.5px**, 상수는 여유 있게 128)에서 멈추고, 본문 시작 위치는 오히려 약 50px 위로 올라옵니다(예전엔 바 102 + 페이지 32 + 알약 줄 + 여백 ≈ 194px 뒤에 내용 시작 → 지금 148px).
- 360px 뷰포트에서 정적 HTML을 Playwright로 렌더해 실제 바 높이·모양을 확인했습니다. DB 변경 없음. lint·build 통과.

## 하단 탭을 스레드식 꽉 찬 바로 + 맨 오른쪽 프로필 (사용자 요청, 스레드 스크린샷: "하단 메뉴가 가로로 길어야 되는 듯. 맨 오른쪽은 프로필 설정 메뉴로")

- **가운데 몰린 알약 → 화면 폭을 채우는 바**: `w-fit`이라 내용 폭만큼만 차지하던 걸 `w-[calc(100%-32px)] max-w-[420px]`로 바꿔 탭이 `flex-1`로 폭을 **균등하게 나눠 갖습니다**(360px에서 한 칸 63px, 430px에서 77px). 둥근 모서리는 `rounded-[26px]`로 살짝 낮춰 긴 바에 맞췄습니다.
- **아이콘 옆 → 아이콘 위, 이름 아래**: 폭이 넓어진 만큼 한 칸 안에 아이콘(24px)과 이름(11px)을 **세로로 쌓았습니다**. 지난 라운드에 역할별로 다르게 잡았던 좌우 여백·글자 크기(`px-[6px]`/`px-[10px]`, 12px)는 필요 없어져 없앴고, 부모·숲지기 모두 같은 모양입니다 — "대시보드"·"추천도서"(4글자)도 63px 칸에 한 줄로 들어갑니다.
- **맨 오른쪽 "프로필" 탭 신설**(`PROFILE_TAB` → `/more`): 아이콘 대신 **지금 프로필의 얼굴**(아이 아바타 / 숲지기는 곰·백로, 상단바와 같은 `face-*.png` 규칙)을 24px 동그라미로 그리고, 그 화면에 있을 때 테두리가 초록이 됩니다. 부모는 오늘·책장·추가·그룹·프로필, 숲지기는 대시보드·아이들·추천도서·숙제·프로필로 **양쪽 다 5칸**이 됐습니다. 상단바의 얼굴+이름도 계속 `/more`로 가지만(기존 요청), 손이 닿는 아래쪽에도 입구가 생겼습니다.
- "추가" 탭의 연두 알약은 그대로 — 이제 세로로 쌓인 칸 전체가 연두 둥근 사각형이 되고, 그 화면에 있을 때 더 진해지는 규칙도 유지됩니다.
- 실제 컴포넌트를 360/430px 뷰포트에서 렌더해 칸 너비(63/77px)와 두 역할의 라벨 줄바꿈 없음을 확인했습니다(임시 라우트, 확인 후 삭제). DB 변경 없음. lint·build 통과.

## 전체 디자인 점검 라운드 (사용자 요청: "전체적으로 디자인적으로 한번 다 점검해줘")

코드 전반을 기계적으로 훑고(모서리 값·색·구분선·글자 크기·컨테이너 여백 분포), 실제 컴포넌트로 대표 화면을 360px에서 렌더해 눈으로 확인한 뒤 고쳤습니다.

- **실제 버그 — 입력창 옆 버튼이 잘림(6곳)**: `<input>`은 flex 항목일 때 `min-width: auto`가 고유 폭(약 200px)으로 잡혀서 **줄어들지 않습니다**. 그래서 `flex-1` 입력창 + 옆 버튼 구조가 카드 안쪽 폭(360px 폰에서 약 268px)을 넘고, `Section`이 `overflow-hidden`이라 버튼이 잘려 나갔습니다(그룹 찾기의 "찾기" 버튼이 실제로 반쯤 잘린 걸 렌더로 확인). 해당 입력창 여섯 곳(`join-by-code`·`child-share`·`shelf-tag-manager`·`shelf-tag-picker`·`read-date-picker`·`library/add`)에 `min-w-0`을 붙여 입력창이 먼저 줄어들도록 했습니다.
- **페이지 시작 여백이 화면마다 달랐습니다**: 대부분 `pt-8`(루트 112.5% 때문에 실제 36px)인데 홈은 12px, 책장은 20px로 따로 튜닝돼 있었습니다. **내용 페이지 전부 `pt-[20px]`로 통일**(22개 파일, 숲지기 세 탭의 `20 + 그룹바 높이` 계산도 함께). 홈만 12px 그대로 — 제목 없이 인사말로 시작하는 화면이라 사용자가 직접 정한 값입니다.
- **페이지 아래 여백**: `pb-10`(45px)이 `main`의 `pb-[96px]`에 더해져 마지막 카드 아래가 141px이나 비었습니다 → `pb-[16px]`로 통일(총 112px, 하단 바 실측 65px + 바닥 14px이라 33px 여유).
- **카드 제목이 페이지 제목과 거의 같은 크기였습니다**: 페이지 `h1` 22.5px / 상단바 20.25px / `Section` 카드 제목 20px — 셋이 2px 안쪽에 몰려 "강약이 없다"던 예전 지적이 재발한 상태였습니다. **카드 제목을 18px**(줄높이 24)로 내리고 카드 머리 여백도 24/24/16 → 24/18/14, 내용 여백 20 → 18로 조였습니다. 이제 22.5(페이지) > 20.25(앱 이름) > 18(카드 제목) > 15.75(본문) > 13.5(보조)로 단계가 벌어집니다.
- **반복되던 색 리터럴을 토큰으로**: 배지 스티커·숲 띠 바탕에 세 번 복붙돼 있던 `#DCE6D0` → `--sprout-pale`.
- **일부러 통일하지 않은 것**: 모서리 값이 여러 개인 건(버튼·입력 `14px` 116곳이 표준, 표지 `8px`, 그룹 타일 `12·14px`, 카드 `var(--r)` 22px, 작은 칩 `10px`) 각자 물리적 크기가 달라 같은 값을 쓰면 오히려 어색해서 그대로 뒀습니다. 스플래시·3D 숲·책등 색의 하드코딩 hex도 그림용 팔레트라 토큰화 대상이 아닙니다.
- 실제 컴포넌트(상단바 + Section 3개 + 진짜 `JoinByCode` + 하단 바)를 360px에서 렌더해 수정 전/후를 비교했습니다(임시 라우트, 확인 후 삭제). DB 변경 없음. lint·build 통과.

## 프로필 화면에서 그룹 나열 제거 (사용자 지적: "프로필 내에서 그룹까진 안 나와도 되겠다")

더보기 → 프로필의 숲지기 프로필 부분이 운영 중인 그룹을 **전부 버튼으로 나열**하고 있었고, 숲지기 프로필이 활성이면 그 버튼들이 **다 같이 "선택됨"**으로 표시돼(그룹마다 고르는 것처럼 보이는데 실제로는 어느 걸 눌러도 같은 동작) 뭘 고르는 건지 알 수 없는 목록이었습니다.

- **`components/operator-profile-switcher.tsx`**: 그룹 버튼 목록과 "+ 새 그룹 만들기"를 빼고, **"운영 중인 그룹 N개 · 대시보드에서 관리 ›"** 한 줄만 남겼습니다(`/teacher`로). 이 화면에서 하는 일은 이제 **숲지기 프로필 자체를 꾸미는 것**(이름·얼굴)뿐입니다 — 아이↔숲지기 전환은 위쪽 `ProfileModeSwitch`, 그룹 고르기는 고정 상단 그룹 바, 그룹 설정·추가는 대시보드의 "운영 중인 그룹"이 이미 맡고 있어 세 곳에 흩어져 있던 중복이 정리됐습니다.
- 안 쓰게 된 `isActive` prop·`selectOperator()`·`GROUP_TYPE_LABELS` import를 지웠습니다. **그룹이 0개일 때는 그대로** 점선 "+ 새 그룹 만들기 (숲지기 프로필이 하나 늘어나요)" — 숲지기가 되는 유일한 입구라 남겨야 합니다.
- DB 변경 없음. lint·build 통과.

## 아이 줄 정렬 + "책장 공유" 기능 제거 (사용자 지적: "들여쓴 너비 안 맞음", "이 기능 없애자")

- **아바타 폭이 제각각이라 이름 시작점이 어긋났습니다**: `AvatarIllustration`은 원본 비율로 폭을 계산하는데(높이 44px 기준 **토끼 17px / 고양이 28px / 강아지 28px**), 더보기의 아이 목록이 그림을 그대로 flex 항목으로 놓고 있어 줄마다 이름이 최대 11px씩 밀렸습니다. 아바타를 **44×44 고정 상자에 가운데 정렬**해 글 칸이 항상 같은 자리(360px에서 x=130)에서 시작하게 했습니다. 숲지기 쪽 아이 목록들은 원래 `h-9 w-9` 원판 안에 넣고 있어 문제가 없었습니다.
- **같은 줄에서 발견한 잘림 두 가지**: 생년월일이 "2019-08-" / "10"으로 꺾이던 것(`truncate`로 한 줄 고정), 오른쪽 "이 아이로 보기"(7글자)가 360px에서 카드 밖으로 잘리던 것 → 줄 왼쪽 전체가 이미 그 아이를 고르는 버튼이므로 **"보는 중" / "›"**로 줄였습니다.
- **"책장 공유"(보호자 초대) 기능을 화면에서 제거**: 더보기의 섹션과 `components/child-share.tsx`를 지웠습니다(`bird-letter` 그림은 배지·숲에서 계속 씁니다). `more`의 `invite_code` 조회도 뺐습니다.
  - **DB는 그대로 둡니다** — `children.invite_code` 컬럼과 `join_child_by_invite_code()` 함수는 남지만 이제 아무 화면도 부르지 않습니다. **마이그레이션 0017의 RLS 조임(새 아이의 첫 보호자만 스스로 등록 가능)은 반드시 유지**해야 합니다: 그게 "child_id만 알면 남의 아이 보호자가 될 수 있던" 구멍을 막은 정책이고, 이번 제거로 두 번째 보호자를 추가하는 경로 자체가 없어졌을 뿐입니다.
- 360px에서 실제 `ChildSwitcher`를 렌더해 세 줄의 이름 시작 x가 모두 같고 날짜·상태가 한 줄에 들어가는 걸 확인했습니다(임시 라우트, 삭제). lint·build 통과.

## 책장 나누기를 책장 탭 안으로 — "책장 정리" 모드 (사용자 요청: "책장 나누기도 굳이 프로필 설정에 있을 필요 없다. 책장 내에서 손쉽게 책장 구분할 수 있게")

프로필·설정에 있던 "책장 나누기"(이름 바꾸기·삭제)는 **책을 나누는 화면이 아니라 이름만 고치는 곳**이었고, 정작 책을 어느 책장에 넣으려면 책 한 권씩 기록 고치기를 열어 "더 남기기"를 펼쳐야 했습니다(30권을 나누려면 30번). 책장 탭 안에서 한 번에 하도록 옮겼습니다.

- **`components/library-shelf.tsx`에 "책장 정리" 모드**: 우측 ⋯ 메뉴 → "책장 정리"로 켭니다. 켜면 **책을 눌러 고르고**(전면·책등·목록 세 보기 모두, 고른 책은 초록 테두리·나머지는 흐리게), 위 패널에서 넣을 책장을 고른 뒤 **"선택한 N권 넣기"** 한 번으로 `reading_records.shelf_tag_id`를 한꺼번에 바꿉니다(`update().in("id", ids)` 왕복 한 번). "모두 선택"은 지금 필터가 걸린 목록 전체를 고릅니다 — 예를 들어 "다 읽었어요"로 거른 뒤 모두 선택해 "6살 책장"에 넣는 식.
- **빼는 것도 같은 버튼**: 책장을 "없음"으로 두면 버튼 문구가 "선택한 N권 책장에서 빼기"로 바뀝니다(따로 버튼을 두면 같은 일을 두 군데서 하게 돼서). 책장은 분류일 뿐이라 기록 자체는 그대로 남습니다.
- **이름 고치기·지우기도 그 패널 안에**: 패널 아래 "책장 이름 고치기 · 지우기 ▾"를 누르면 기존 `ShelfTagManager`가 펼쳐집니다. `app/more`의 "책장 나누기" 섹션은 제거.
- **`ShelfTagPicker`에 `label` prop**: 기록 화면에선 "어느 책장에 꽂을까요? (선택)", 정리 패널에선 "어느 책장에 넣을까요?"로 묻는 말이 달라야 해서 갈아 끼울 수 있게 했습니다.
- 360px에서 실제 `LibraryShelf`를 렌더해 ⋯ 메뉴 → 정리 모드 → 두 권 선택까지 눌러 확인했습니다(임시 라우트, 삭제). DB 변경 없음.

## 아이 프로필로 보는 중에 숲지기 화면이 열리던 것 (사용자 지적: "어린이 계정 선택한 상태에서 숲지기 대시보드 관리 들어가면 안 될 거 같은데 들어가지네, 좀 이상하게 보이고")

숲지기 화면들(`/teacher/**`)은 "이 계정이 그룹을 운영하는가"만 확인하고 있어서, **아이 프로필로 보는 중에도** 링크만 타면 대시보드가 열렸습니다 — 그런데 하단 탭은 아이 탭(오늘·책장·추가·그룹) 그대로라 화면이 섞여 보였습니다. 그룹 상세 화면에 이미 적용해 둔 규칙(관리 화면은 운영진 **+ 숲지기 프로필일 때만**)을 숲지기 화면 전체로 넓혔습니다.

- **`app/teacher/layout.tsx`(신규)**: `users.active_profile_type`이 `operator`가 아니면 본문 대신 "지금은 아이 프로필로 보고 있어요 · 숲지기 프로필로 전환하기" 안내를 보여줍니다. 레이아웃 한 곳이라 `/teacher` 아래 모든 화면(대시보드·아이들·추천도서·숙제와 그 상세·만들기 화면)이 한 번에 지켜지고, 조회는 컬럼 하나짜리 한 줄이라 각 화면 조회와 병렬로 돕니다.
- **`components/switch-to-operator.tsx`(신규)**: 그 안내 화면의 전환 버튼(프로필을 바꾸고 `/teacher`로).
- **프로필 화면의 "대시보드에서 관리 ›"도 전환을 먼저**: 그냥 링크였던 걸 버튼으로 바꿔 `active_profile_type`을 숲지기로 바꾼 뒤 대시보드로 갑니다 — 이제 이 경로로는 위 안내 화면을 만날 일이 없습니다.
- DB 변경 없음. lint·build 통과.

## 책장 탭 첫 드롭다운이 "그룹"인데 책장도 나오던 것 (사용자 지적)

책장 탭의 필터 드롭다운 세 개 중 첫 번째가 라벨만 "그룹"이고 실제 목록엔 **직접 나눈 책장(6살 책장 등)과 그룹(학급·기관)이 한 줄로 섞여** 있었습니다 — 예전에 "출처" 줄과 "책장" 줄을 하나로 합치면서 라벨을 "그룹"으로 바꿔 달라는 요청만 반영했던 자리라, 이름과 내용이 어긋난 상태였습니다.

- **드롭다운은 그대로 하나, 안에서 두 묶음으로**: `<optgroup label="내 책장">`(직접 나눈 책장)과 `<optgroup label="그룹">`(학급·기관)으로 갈랐고, 닫힌 상태 라벨은 **"책장·그룹"**으로 바꿨습니다(`aria-label`도 같이). 둘을 각각 드롭다운으로 쪼개면 한 줄에 네 개가 되어 더 복잡해지므로(사용자도 "드롭다운이 너무 많아질 것 같다"고 짚음) 머리글로만 나눴습니다. 해당 종류의 기록이 하나도 없으면 그 `optgroup` 자체가 안 뜹니다.
- **폭**: 라벨이 길어져 첫 드롭다운 80 → 88px, 대신 줄 간격 `gap-2` → `gap-1.5`. 360px에서 실측(정적 HTML + Playwright) 시 세 드롭다운 88+92+76에 구분선 13px이 남아 한 줄에 들어갑니다.
- DB 변경 없음. lint·build 통과.

## 숲지기 추천도서 세 가지 보기 + 분야 필터 + 순번 + 내보내기(엑셀·인쇄) (사용자 요청)

숲지기의 추천도서 탭이 "날짜·칩·제목" 줄 목록 하나뿐이라, 아이 쪽 책장 탭에 이미 있는 기능들(보기 방식 전환·분야로 좁혀 보기)이 없었습니다. 네 가지를 한 번에 반영했습니다.

- **`lib/shelf-visual.ts`(신규)**: 나무 선반(`PLANK_STYLE`)·책등 높이·책등 색·`chunk`를 `components/library-shelf.tsx`에서 빼내 평범한 모듈로 옮겼습니다(서버 컴포넌트에서도 안전하게 import되도록 — `"use client"` 파일에서 상수를 내보냈다가 프록시 객체로 넘어와 레이아웃이 깨진 전례가 있어 규칙대로). 아이 책장과 숲지기 화면이 같은 선반을 씁니다.
- **`components/operator-book-browser.tsx`(신규)**: 추천도서 탭의 본체.
  - **보기 방식 드롭다운**(목록·전면·책등, `localStorage`에 저장) — 전면은 선반 위 표지 3권씩, 책등은 8권씩 한 선반. 둘 다 누르면 그 책의 상세(아이별 읽은 현황)로. **관리(추가·선택·삭제)는 목록 보기에만** 둡니다(표지만 훑는 화면에서 삭제 체크박스는 어울리지 않음).
  - **분야 필터 칩**: 실제 올린 책에 붙어 있는 분야만, 둘 이상일 때만 줄이 뜹니다. 이미 고른 칩을 다시 누르면 전체로(책장 탭과 같은 토글 방식).
  - **"N권 ── 보기 ▾ · 내보내기"** 컨트롤 줄(책장 탭의 줄과 같은 모양).
- **목록 줄 맨 왼쪽에 순번**: `LogRow`에 `index` prop을 더해 **날짜 칸 위에 작게** 그립니다 — 번호 칸을 따로 만들었더니 그만큼 제목 칸이 좁아져 "이상한 엄마"가 두 줄로 꺾이는 걸 360px 렌더로 확인하고, 칸을 늘리지 않는 이 방식으로 바꿨습니다. 같은 날짜라 날짜를 숨기는 줄에도 번호는 계속 보입니다.
- **`app/teacher/export`(신규) + `components/export-buttons.tsx`(신규)**: 추천도서(`?type=books`)·숙제(`?type=assignments`)를 표로 보여주고 **엑셀(CSV) 내려받기**와 **인쇄·PDF** 두 버튼을 답니다. CSV는 엑셀이 한글을 안 깨뜨리도록 BOM + CRLF, 모든 칸을 따옴표로 감쌉니다. 화면의 표와 CSV가 같은 `string[][]` 하나에서 나와 서로 어긋날 수 없습니다. 열은 추천도서 = 번호·올린 날·분야·제목·저자·읽은 아이·숙제 중, 숙제 = 번호·낸 날·마감·제목·책·미션·완료.
- **`lib/operator-assignments.ts`(신규)**: 숙제 탭에 인라인으로 있던 조회(임베드 한 번 + `assignment_completion`을 아이 단위로 묶기)를 빼내 숙제 탭과 내보내기 화면이 공유합니다. 숙제 탭 상단에도 "숙제 N개 ── 내보내기" 줄을 붙였습니다.
- iPhone 360px에서 세 보기를 실제로 전환해 확인했습니다(임시 라우트, 삭제). DB 변경 없음.

## 대시보드 각주 문구 + 하단 탭 칸 사이 간격 (사용자 지적)

- **"여러 그룹에 속해도 아이는 한 명으로 세어요" → "여러 그룹에 속하는 한 아이는 한 명으로 세어요"**: 앞 문장은 "누구를 한 명으로 세는지"가 모호했습니다.
- **하단 탭에 `gap-[4px]`**: 켜진 탭의 초록 알약과 상시 연두색인 "추가" 알약이 딱 붙어 한 덩어리처럼 보인다는 지적. 칸 사이를 4px 띄웠습니다 — 360px에서 한 칸 62px, 가장 긴 라벨("대시보드") 44px이라 여유가 남는 걸 실측으로 확인했습니다.

## 기록 화면 날짜 칸·숙제 줄 음영·저장 후 스크롤 (사용자 피드백 3건)

- **"읽고 싶어요"일 때 "언제 읽었어?"를 못 누르게**: 아직 안 읽은 책인데 읽은 날짜를 고를 수 있는 게 앞뒤가 안 맞았습니다. `ReadDatePicker`에 `disabled`/`disabledHint`를 더해 흐리게(0.45) 두고 버튼·입력을 잠급니다(사라지진 않게 — 상태를 바꾸면 바로 쓰던 자리 그대로). 제목 옆에 "다 읽고 나서 골라요". 기록 남기기·기록 고치기 두 곳 모두.
- **날짜가 "2026. 9." / "12."로 두 줄 되던 것**: 오늘·어제·그제 세 버튼과 날짜 입력을 한 줄에 넣어서, iOS가 그리는 "2026. 9. 12."가 좁은 칸에서 꺾였습니다. **빠른 버튼 한 줄 + 날짜 입력 한 줄(전체 너비)**로 나눴습니다 — 칸 너비를 조금 늘리는 방식은 iOS 렌더 폭을 이 환경에서 확인할 수 없어 확실한 쪽을 택했습니다.
- **숙제 제목 줄에 옅은 음영 띠**: `components/assignment-today.tsx`에서 숙제 제목·안내 줄만 `--paper` 배경으로 감쌌습니다(그 아래 책 줄은 흰 바탕 그대로) — 제목과 책 목록이 한 덩어리로 읽혀 어디까지가 숙제 이름인지 헷갈린다는 지적. 그 줄의 "N/M" 배지는 띠 위에서 묻히지 않게 흰 바탕으로.
- **저장 직후 발자국 도장이 안 보이던 것**: 폼을 한참 내려 놓은 채 저장하면 완료 화면이 스크롤 위에 가려 있었습니다. `setStep("saved")` 직후 `window.scrollTo({ top: 0 })`.

## 숲지기 프로필 줄을 아이 줄과 같은 모양으로 + 전환 전 확인창 (사용자 요청)

- **`components/operator-profile-switcher.tsx` 재구성**: 곰·백로 칩 두 개가 늘 펼쳐져 있던 걸 접고, **아이 프로필 줄과 같은 한 줄**로 만들었습니다 — 지금 고른 얼굴(44px 동그라미) + 숲지기 이름 + "그룹 N개", 오른쪽에 **고치기**와 상태("보는 중" / "›").
  - **줄을 누르면 그 자리에서 숲지기로 전환**하고 대시보드로 갑니다(예전엔 "대시보드에서 관리" 버튼이 따로 있었는데 같은 동작이라 하나로 합침).
  - **고치기**를 누르면 이름 입력 + 곰·백로 고르기가 함께 펼쳐지고, **저장 한 번에 둘 다** 반영합니다(`users.operator_name`·`operator_avatar` + 내 그룹들의 `operator_name` 복사본).
  - `isActive` prop이 다시 필요해져 `app/more/page.tsx`에서 넘깁니다.
- **프로필 전환 전 확인창**: 아이 줄(`child-switcher.tsx`)·숲지기 줄·위쪽 전환 스위치(`profile-mode-switch.tsx`) 세 곳 모두 `window.confirm("{이름} 프로필로 전환하시겠어요?")`를 먼저 띄웁니다 — 화면 전체(하단 탭·기록 대상)가 바뀌는 큰 동작이라 실수로 눌리는 걸 막습니다. 이미 보고 있는 프로필을 누르는 건 그냥 그 첫 화면으로 가므로 묻지 않습니다.
- 360px에서 실제 컴포넌트로 기본/보는 중/고치기 세 상태를 확인했습니다(임시 라우트, 삭제). DB 변경 없음.

## 모바일 UI 전면 점검 브리프 재점검 — 오늘 탭 통계 4칸 → 3칸 (사용자 요청: "전체적으로 모바일 UI를 정돈해줘")

이전에 8개 항목(글씨체 위계·여백 체계·색상·오늘 화면·숙제 화면·책장 화면·하단 내비게이션·검증)으로 받은 종합 정돈 브리프를 실행하기 전에, 그 사이(같은 세션의 앞선 turn들에서) 이미 진행된 수십 차례의 개별 라운드가 이 브리프의 항목 대부분을 실질적으로 충족하고 있는지부터 현재 코드로 다시 확인했습니다.

- **이미 충족된 것으로 확인**: 손글씨(`.hand`)는 인사말·숲 띠 문구 두 곳으로만 제한(규칙 명문화됨), 여백은 8px 단위 체계로 정리(`전체 디자인 점검 라운드`), 카드 모서리·그림자·배지 알약 모양 통일, 숙제 화면은 흰/연한 배경/흰 3영역 구조·날짜 칸 분리·표지 포함·긴 제목 2줄 클램프·완료 배지 겹침 없음(`components/assignment-today.tsx`, `components/log-row.tsx` 확인), 책장 화면은 선반 정렬·검색창 축소 방지·컨트롤 줄 정리, 하단 내비게이션은 켜진 탭만 강조 + 이름 상시 표시 + 안전영역 여백 확보(`components/bottom-nav.tsx`) — 모두 브리프 이후가 아니라 그 이전의 여러 라운드에서 이미 반영돼 있었습니다.
- **실제로 남아 있던 gap 하나**: `app/today/page.tsx`의 요약 카드 통계가 여전히 "오늘·이번 주·이번 달·그룹" 4칸이었습니다. 브리프가 명시적으로 요청한 "통계는 오늘·이번 주·이번 달 3항목만 — 그룹 수 제거"를 반영해 "그룹" 칸(과 그 계산에만 쓰이던 `group_members` 조회)을 없애고 `grid-template-columns`를 3열로 바꿨습니다. 그룹 목록으로 가는 입구는 우리 숲 미리보기와 하단 "그룹" 탭에 이미 있어 기능 손실은 없습니다. 정적 HTML 복제본을 360/430px에서 Playwright로 렌더해 3칸이 고르게 배치되고 카드 높이가 줄어드는 걸 확인했습니다.
- **의도적으로 브리프와 다르게 둔 것 두 가지**: (1) "+ 책 기록하기" 버튼은 브리프가 "주요 CTA는 짙은 숲 초록으로 통일"을 요청하지만, 이 색(`--berry`, 호박색이 아니라 레드)은 그보다 앞서 사용자가 이 버튼 하나를 콕 집어 명시적으로 요청한 색이라(레거시 문구 항목 참고) 되돌리지 않았습니다. (2) 하단 탭의 "추가"는 브리프가 "작은 원형 + 버튼"으로 다르게 하라고 제안했지만, 실제로는 그 이후 여러 라운드에 걸쳐 사용자가 직접 "항상 연두 알약으로" 등 지금 모양을 구체적으로 요청해 확정한 상태라 그대로 뒀습니다. 두 경우 모두 브리프보다 더 나중에, 더 구체적으로 확정된 사용자 결정이라 우선했습니다.
- `npm run lint`, `rm -rf .next && npm run build` 통과. DB 변경 없음.

## 전체 검색(핵심 기능) + 숙제 카드 전면 개편 + 그룹 탭/하단 메뉴 정리 (사용자 요청 — 대형 브리프)

"모바일 UI와 숙제 이용 흐름을 개선사항에 맞게 수정해줘. 기존 기능·데이터·선생님 원문·기존 완료 조건은 보존" 이라는 6개 항목짜리 큰 브리프를 받았습니다. 새 기능(전체 검색)과 기존 화면 재구성(숙제 카드, 그룹 탭, 오늘 탭, 하단 메뉴)을 함께 진행했고, 마이그레이션 없이 순수 애플리케이션 코드만 바꿨습니다(완료 조건·데이터 모델 변경 없음).

### 1) 전체 검색 — `/search` (신규, "핵심 기능")
- **`lib/search.ts`의 `searchAll()`**: 아이가 승인된 그룹 전부(가입한 모든 그룹)를 대상으로, **책 제목·작가·숙제 제목·숙제 설명·질문·추천 설명(책 소개/요약)·그룹명**을 검색합니다. 완료·지난 자료도 날짜로 거르지 않고 전부 대상입니다(진행 중/완료·지난만 보기는 선택적 필터). RLS(그룹 멤버십)가 이미 "이 아이가 볼 수 있는 것"만 돌려주므로 별도 권한 체크 없이 "열람 권한을 가진 전체 데이터"가 됩니다.
- **정확한 단어 검색**: 공백으로 나눈 단어 전부가 한 필드 안에 단어 경계로(앞뒤가 글자/숫자가 아님, 유니코드 기준) 등장해야 매치합니다(형태소 분석 없는 1단계 구현, 브리프에서 요청한 대로 "우선 정확한 단어 검색부터"). 필드별로 개별 매치를 판정해서, 제목에 걸리면 "숙제", 설명에 걸리면 "숙제 설명", 질문에 걸리면 "질문", 책 정보에 걸리면 "숙제 책"/"추천도서"/"추천 설명"으로 결과 종류가 따로 나옵니다.
- **결과 표시**: 표지·제목·그룹명·날짜·매치 문장 스니펫(앞뒤 문맥 포함)·종류 배지. 누르면 원래 위치(숙제 카드, 특정 질문, 특정 추천도서)로 앵커(`#assignmentId`, `#mission-{id}`, `#book-{itemId}`) 이동합니다 — 이를 위해 `assignment-today.tsx`의 미션 행과 `recommend-shelf.tsx`의 책 타일에 `id`+`scrollMarginTop`을 추가했습니다(고정 그룹 바에 안 가리게).
- **그룹·종류·기간 필터**: `components/search-bar.tsx`가 그룹/종류(전체·숙제만·추천도서만)/기간(전체·진행 중만·완료·지난 자료만) 드롭다운을 제공하고, 전부 URL 쿼리(`?q=&group=&type=&period=`)로 저장합니다 — 결과를 누르고 뒤로 와도, 새로고침해도 검색어·필터·스크롤이 그대로입니다(브라우저 히스토리 자체가 상태라 별도 복원 로직이 필요 없습니다). 결과가 없을 때는 "N개 그룹의 숙제·추천도서·질문·설명을 확인했다"는 안내와 함께 검색어를 바꿔보라고 안내합니다.
- **`components/search-results.tsx`**: 결과 목록 렌더링을 별도 컴포넌트로 빼서 `/search` 페이지와 검증용 미리보기가 공유합니다.
- **진입점**: 그룹 탭(`/group`) 상단 고정 바의 밑줄 탭 오른쪽에 돋보기 아이콘, 그룹 탭 안의 "이 목록에서 빠르게 찾기" 검색창 아래 "전체 숙제·추천도서 검색(완료·지난 자료 포함) ›" 링크.

### 2) 숙제 카드 전면 개편 (`components/assignment-today.tsx`)
정보 순서를 브리프가 지정한 그대로 재구성했습니다 — **마감일(크게) → 등록일·그룹명(작게) → 숙제 제목·설명(연한 배경, 선생님 원문 그대로) → 책 목록(흰 배경, 1단계) → 질문·낭독(2단계)**. 기존 `LogRow`/`LogGroup`(마감일별로 여러 그룹 숙제를 한 헤딩에 묶던 구조)은 각 카드가 자기 마감일을 크게 보여줘야 하는 새 레이아웃과 맞지 않아 이 컴포넌트에서만 뺐고, 카드 하나하나가 독립된 카드가 됐습니다(검색 결과 앵커 대상이기도 해서 이 편이 더 명확합니다).

- **마감일**: "9월 13일(일)까지" 형식(`lib/assignment-period.ts`의 `formatDueLong`), 옆에 "오늘까지"(호박색)/"내일까지"(초록)/"기한 지남"(레드) 배지(`dueBadge()`). 완료된 숙제는 이 배지 대신 "숙제 완료" 배지만 보여줍니다(급함 표시와 완료 표시가 같이 있으면 헷갈려서).
- **책 한 줄(`HomeworkBookRow`)**: 큰 표지(64×80, 기존 44×32보다 확대) + 제목·작가 + **체크 아이콘 대신 실제 버튼** — 안 읽음이면 "읽기 시작"(기록 화면으로, 초록 채움) + "다 읽었어요"(빠른 완료, 초록 테두리) 두 버튼, 읽음이면 "다 읽었어요" 배지 + "완료 취소" 텍스트 버튼. "완료 취소"는 기존 `lib/quick-read.ts`의 `setRead(false)`를 그대로 재사용해 `done → want`로 되돌립니다(새 로직 아님, 기존 데이터 구조·권한 그대로).
- **"2권 중 1권 읽었어요"**: `{개수}/{전체}` 대신 문장형으로 바꿨습니다.
- **1단계/2단계**: 책과 질문(또는 낭독) 미션이 함께 있을 때만 "1단계 · 책 읽기" / "2단계 · 질문 확인 및 답변 작성" 소제목을 붙입니다.
- **질문 미션(`QuestionMission`)**: 기본은 접혀 있고 상태에 맞는 버튼만 — 답 없음 "답 적기"(누르면 입력창), 답 있음 "내 답 보기"(누르면 답 펼치고 옆에 "답 수정"). 체크 아이콘이나 상시 열린 입력창 대신 실제 상태 버튼입니다.
- **완료 조건은 손대지 않았습니다**: `isAssignmentDone()`(새로 내보낸 헬퍼, 로직은 기존과 동일 — 책이 하나 이상이고 전부 `completed`)이 "읽기 완료"(책 단위)와 "숙제 전체 완료"(과제 단위)를 구분합니다. `assignment_completion` 뷰의 완독/부분 읽기(쪽수) 판정은 전혀 안 건드렸습니다.

### 3) 숙제 목록 상태별 정리 (`components/assignments-browser.tsx`)
예전의 "이번 주 숙제/다가오는 숙제" 날짜 구간 나열을 없애고, 완료 여부·기한 기준 3단으로 바꿨습니다 — **지금 해야 할 숙제**(미완료, 마감 가까운 순) → **기한 지남**(마감이 지난 미완료 — 숨기지 않고 별도 표시, 레드) → **완료한 숙제**(전부 완료, 최근 마감부터). "지난 숙제 전체 보관함"(`/group/past`)은 그대로 남겨 더 오래된 기록을 훑어볼 수 있게 했습니다. 이 화면 안의 검색창은 "지금 로드된 목록"만 빠르게 걸러주는 보조 기능이고, 완료·지난 자료를 포함한 진짜 전체 검색은 `/search`로 안내합니다.

### 4) 오늘 탭 숙제 요약 (`components/assignment-summary.tsx`)
텍스트 한 줄("제목 + N/M 완료")이던 걸 **대표 책 표지 + 마감일(+ 급함 배지) + "N권 중 M권 읽었어요" + 다음 행동 버튼**으로 확장했습니다. 다음 행동은 상황에 따라 자동으로 정해집니다 — 안 읽은 책이 있으면 그 책 "읽기 시작", 책은 다 읽었는데 답 안 한 질문이 있으면 "답 적기"(해당 질문 앵커로), 전부 끝났으면 "완료 · 다시 보기". 오늘 탭 자체는 여전히 요약이고, 책마다 전부 펼쳐 보여주는 상세는 그룹 탭이 맡습니다.

### 5) 그룹 탭 상단 바 (`components/child-group-bar.tsx`)
- **큰 이미지 + 큰 숫자, 이름은 작게**: 타일을 44px→56px로 키우고, 지금 보는 소제목 탭(숙제/추천도서) 기준 개수를 타일 오른쪽 아래 배지로 크게 보여줍니다(숙제 탭이면 그 그룹 숙제 수, 추천도서 탭이면 책 수). 이름은 그 아래 10px로 작게. 선택 상태는 테두리 색 + 불투명도 변화로 뚜렷하게 구분됩니다. 그룹이 많으면 가로 스크롤.
- **밑줄형 두 칸 탭**: 숙제/추천도서 소제목 탭을 알약 버튼에서 배민 스타일 밑줄 탭(화면을 반씩 나눠 갖고, 선택된 쪽만 굵은 글씨 + 아래 초록 밑줄)으로 바꿨습니다. 오른쪽 끝에 전체 검색(`/search`)으로 가는 돋보기.
- **그룹·탭 상태 유지**: 그룹 선택(`children.active_group_id`)과 소제목 탭(`?tab=`)은 그대로 URL/DB에 있어 서로 오가도 초기화되지 않습니다(기존 설계 그대로, 이번에 깨지지 않았는지 재확인).
- 정적 HTML 복제본을 360/390/430px에서 Playwright로 실측(125px)해 `lib/group-bar-height.ts`의 `CHILD_GROUP_BAR_HEIGHT`를 140으로 올렸습니다(본문이 고정 바 뒤에 숨는 사고가 전에 한 번 있었어서 여유 있게).

### 6) 추천도서는 완료를 요구하지 않음 (`components/recommend-shelf.tsx`)
표지 위 완료 체크(`ReadCheck`) 오버레이를 뺐습니다 — 이제 표지(누르면 "읽어보기"=기록 화면)와 책갈피(담아두기)만 남아, 숙제와 달리 완료를 강요하지 않는 탐색 전용 화면이 됐습니다. 표지가 없을 때 대체 표시(제목만 있는 색 판)에서 긴 제목이 박스 밖으로 잘리던 것도 5줄 클램프로 같이 고쳤습니다.

### 7) 하단 메뉴 — "추가"를 독립 버튼으로 (`components/bottom-nav.tsx`)
"추가"를 탭 강조 배경을 공유하는 알약 목록에서 빼고, 알약 바 위에 살짝 떠 있는(raised) 별도의 연두색 원형 버튼으로 분리했습니다 — 탭이 아니라 "행동 버튼"이라는 게 형태로도 드러납니다. 나머지 탭(오늘·책장·그룹·프로필)은 그대로 "지금 화면만 배경으로 강조"를 유지합니다. `app/layout.tsx`의 `<main>` 하단 여백을 96px→112px로 늘려 이 버튼이 스크롤 맨 아래 내용을 가리지 않게 했습니다.

### 8) 주요 버튼 색 통일
"주요 버튼은 짙은 숲 초록으로 통일해줘"라는 이번 브리프의 명시적 지시에 따라, 오늘 탭의 "+ 책 기록하기" 버튼을 예전에 한 번 콕 집어 요청받았던 레드(`--berry`)에서 `--point-deep`(짙은 초록)으로 되돌렸습니다 — 더 최신의 명시적 지시를 우선했습니다. 삭제·경고류 텍스트(에러 메시지, 삭제 버튼, 기한 지남 배지 등)의 berry는 원래 취지(드물게, 경고·삭제 전용)대로 그대로 뒀습니다.
...
## 숲지기의 아이 상세 화면 — 추천도서 100권도 트래킹 안 하게 접기 + LogRow 날짜 위치 (사용자 지적, 스크린샷 2장)

- **`/teacher/children/[childId]`(아이 상세)의 추천도서 목록이 너무 소모적이었습니다**: 그룹당 추천도서가 100권 가까이 될 수 있는데, 이 화면이 책마다 표지·등불("숙제 중")·발자국 도장·색깔 상태 글자를 전부 늘어놓고 있었습니다("이걸 다 트래킹하는 건 안 좋아 ... 소모할 필요 없음"). `components/child-book-tracker.tsx`(신규, 클라이언트)로 빼서 **기본은 접힌 상태**로 두고("N/M권 읽음"은 `Section` 머리글에 그대로), "책별로 자세히 보기 ›"를 눌러야 펼쳐지도록 했습니다. 펼친 목록도 아이 화면의 화려한 배지(발자국 도장, 등불 아이콘, 색깔 텍스트) 대신 **제목 + 상태만 있는 옅은 표 형태**로 단순화했습니다 — "이건 부모(숲지기)가 보는 관리 화면이라 아이 모드처럼 꾸밀 필요 없고 오히려 체계화가 필요하다"는 지적 반영. `inAssignment`(숙제 중) 표시는 정보 손실 없이 제목 옆 회색 텍스트로만 남겼습니다.
- **`components/log-row.tsx`의 날짜 위치가 "쌩뚱맞다"는 지적**: 숲지기의 추천도서·숙제 관리 목록(`components/managed-log-list.tsx`, `components/recommend-book-list.tsx`가 공유하는 `LogRow`)이 날짜(예: "9/10")를 줄 맨 왼쪽 전용 칸에 두고 있었는데, 오른쪽 위(상태·인원수 배지 바로 위)로 옮겼습니다. 왼쪽 칸이 없어지면서 색 칩이 줄 맨 앞으로 당겨지고, 구분선도 이제 줄 전체 폭을 그대로 씁니다(예전엔 날짜 칸을 피해 칩부터 시작했는데, 그 칸 자체가 없어져 자연스럽게 정리됨). `LogRow`는 두 컴포넌트(둘 다 숲지기 전용 화면)에서만 쓰여 아이·부모 화면에는 영향이 없습니다.
- 임시 미리보기 라우트로 `ChildBookTracker`(접힘/펼침)와 `LogRow`(날짜 위치, 같은 날짜 줄의 `hideDate`)를 렌더링해 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 숙제 상태 점 — 완료는 색만이 아니라 모양으로도(초록 원 + 흰 체크) (사용자 스펙: 표로 정리해 전달)

바로 위에서 그룹 타일·하단 탭의 숙제 개수 배지를 상태 점(호박색/초록)으로 바꿨는데, 사용자가 표로 명확한 스펙을 다시 줬습니다 — 남은 숙제 있음=호박색 점, 모두 완료=**초록 원 안에 흰색 체크**(색만 다른 점이 아니라 모양도 달라야 함), 배정된 숙제 없음=표시 없음.
...
## 기록 고치기 — 삭제 버튼이 안 먹히던 문제 + 분야 태그 추가 (사용자 신고: "이 기록 삭제하기 버튼 안 먹히네. 기록 할때 카테고리도 선택 가능하게 도 안되어있네")
...
## 배지의 곰·새를 다른 그림으로 교체 + 숲지기 캐릭터가 그룹마다 실제로 고른 얼굴로 (사용자 요청: "곰과 새는 빼고 다른 모양으로... 곰과 새는 그룹 숲지기와 연동해서 숲지기 수만큼, 그 캐릭터에 따라 늘어나는 걸로")

바로 앞 항목("첫 숲지기" 곰/새 불일치)을 고친 뒤, 사용자가 더 큰 방향을 요청했습니다 — 곰·새 그림은 이제부터 **오직 "그룹(숲지기) 몇 곳에 속해 있는가"를 보여주는 자리에만** 쓰고, 사진·목소리·이야기꾼처럼 곰·새와 무관한 배지들은 곰·새 대신 새로운 그림으로 바꿔서 "모으는 재미"가 다양해지게 해달라는 것과, 숲길의 곰·새가 실제로 각 그룹이 고른 얼굴(곰 또는 백로)을 반영하게 해달라는 것 두 가지입니다. 사용자가 참고로 보내준 인터넷 수채화 그림은 저작권이 불분명해 그대로 쓰지 않고("살짝 따라만 해줘"), 톤·소재만 빌려 새로 그린 단순한 벡터 아이콘으로 만들었습니다(이 프로젝트의 "이모지·클립아트 금지, 커스텀 오리지널 그래픽만" 원칙 그대로).

- **`components/icons/badge-critters.tsx`(신규)**: 나비(`ButterflyIcon`)·무당벌레(`LadybugIcon`)·달팽이(`SnailIcon`) 세 가지를 손으로 그린 단순 SVG로 새로 만들었습니다. 사진 배지("장면 수집가"/"장면 앨범")는 나비, 목소리 배지("숲의 목소리"/"숲의 합창")는 무당벌레, 이야기꾼 배지("이야기꾼 열/서른 명")는 달팽이(이야기를 등에 지고 천천히 오래 간다는 은유)로 배정했습니다. `components/badge-art.tsx`의 `CRITTER_ART` 표가 이 셋을 그리고, 곰(그룹1/3)·발자국(재독)·나무(권수)·별·등불은 그대로입니다.
- **`lib/forest-scene.ts`**: `OrnamentKind`에 `butterfly`/`ladybug`/`snail`을 추가하고, `photo10/30 → butterfly`, `voice5/20 → ladybug`, `author10/30 → snail`로 매핑해서 우리 숲(2D·3D)의 장식도 배지 목록과 항상 같은 그림을 그리게 했습니다("첫 숲지기" 사고를 다시 겪지 않기 위해 배지-장식 매핑을 항상 짝 맞춰 바꾸는 규칙을 확인). `components/forest-ornament.tsx`(2D)와 `components/forest-3d.tsx`(3D, `Butterfly3D`/`Ladybug3D`/`Snail3D` — 기존 클레이 스타일의 단순 도형 조합)에 렌더링을 추가했습니다.
- **숲지기 캐릭터가 그룹마다 실제로 고른 얼굴로**: 지금까지 숲길 끝의 곰은 그룹이 무엇을 골랐든 항상 곰 하나였습니다(더보기 → 숲지기 프로필에서 곰/백로 중 고르는 기능 자체는 있었지만, 우리 숲 화면에 반영되지 않고 있었음). 마이그레이션 0028로 `groups.operator_avatar`(`users.operator_avatar`의 복사본, `operator_name`과 같은 패턴)를 추가하고, `operator-profile-switcher.tsx`의 저장 로직이 이름과 함께 얼굴도 내 그룹 전부에 복사하도록 고쳤습니다. `lib/badge-data.ts`의 `loadForestData()`가 그룹마다 `avatar`를 함께 돌려주고, `lib/keeper-scene.ts`(`keeperIllustration()`, `KEEPER_MAX=4`)가 "곰이면 bear-lantern, 백로면 bird-perched"를 정합니다. `components/forest-view.tsx`/`forest-strip.tsx`가 그룹마다 한 명(최대 4명)을 그 얼굴로 세우도록 바꿨고, 예전의 "1곳/3곳 문턱" 방식(`keeperBearCount`)은 지웠습니다 — 이번엔 서로 다른 그룹이 서로 다른 얼굴로 뚜렷이 구분되니 "그룹 하나 늘 때마다 계속 늘어 헷갈린다"던 예전 문제(그래서 문턱 방식을 도입했었음)가 재발하지 않을 것으로 판단했습니다.
- **3D 숲(`components/forest-3d.tsx`)도 같은 규칙**: 그룹마다 곰(`BearWithLantern`) 또는 백로(`EgretKeeper`, 기존 `BirdBody`를 키워 재사용 — 전용 서 있는 모델은 아직 없어 임시로 대체) 중 그 그룹이 고른 쪽을 세우고, 하늘을 크게 도는 백로(`SkyBird`)는 백로를 고른 그룹에만 하나 더 얹습니다(곰 숲지기는 땅에 서 있는 것으로 충분). `forest-3d-screen.tsx`의 요약 문구도 새 장식 종류(나비·무당벌레·달팽이·곰)를 전부 세도록 일반화했고, 살아있는 것은 "마리" 사물은 "개"로 세는 단위 표(`ORNAMENT_COUNTER`)를 추가했습니다.
- **의도적으로 단순화한 부분**: 백로 숲지기의 3D 모델은 전용으로 새로 만들지 않고 기존 새 그림(BirdBody)을 키워 썼습니다 — 실제 걷는 자세의 백로 전용 모델은 다음에 그림/모델이 생기면 교체할 수 있습니다.
- 임시 미리보기 라우트(확인 후 삭제)로 배지 목록의 나비·무당벌레·달팽이 아이콘과, 곰 숲지기 그룹·백로 숲지기 그룹을 하나씩 넣어 숲길 끝에 곰과 백로가 나란히 서는 것을 스크린샷으로 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. **SQL Editor에서 마이그레이션 0028 실행 필요**(`groups.operator_avatar` 컬럼 추가).
- **마이그레이션을 아직 안 돌린 상태에서 실제로 겪은 부작용**: 사용자가 그룹 1곳에 속해 있는데도 "첫 숲지기" 배지가 안 딴 것으로 나온다고 신고했습니다. 원인은 `loadForestData()`의 `group_members` 조회가 `groups(name, operator_avatar)`를 임베드하는데, 아직 없는 컬럼이라 PostgREST가 이 select 자체를 오류로 돌려주고, 코드가 그 오류를 확인하지 않은 채 `data`만 보고 있어서(`memberRows`가 `null`) **그룹이 통째로 0곳으로 조용히 계산**되고 있었습니다 — 예전에 `getActiveChild()`에서 겪었던 것과 완전히 같은 유형의 사고("조회 실패가 빈 상태로 위장")가 이번 컬럼 추가로 재발한 것입니다. `lib/badge-data.ts`가 이제 그 오류를 확인해서, 있으면 `operator_avatar` 없이 한 번 더 조회하는 방어 로직을 추가했습니다 — 마이그레이션 전에도 그룹 수·이름은 정확히 세어져 "첫 숲지기"류 배지가 정상 작동하고, 얼굴만 기본값(곰)으로 보입니다(마이그레이션을 돌리면 그 순간부터 실제로 고른 얼굴이 반영됩니다). 같은 이유로 `operator-profile-switcher.tsx`의 저장 로직도 `groups.operator_avatar` 업데이트가 실패하면(컬럼 없음) 이름만이라도 반영되도록 한 번 더 시도하게 고쳤습니다 — 안 그러면 마이그레이션 전에는 숲지기 이름을 바꿔도 그룹 쪽에 전혀 안 바뀌는 문제가 있었습니다.


## 사용자가 직접 그린 수채화 배지 그림 9종 연결 (사용자 요청: "배지 많이 만들어왔다. 내가 그린거라 그냥 써도됨. 나무 배지 말고도 다양한거 넣어줘")

사용자가 직접 그린 수채화 그림 9장(물망초·할미꽃 다발, 도토리, 버섯 무리, 여우, 랜드로버 디펜더, 캠핑 트레일러, 티피 텐트, 티백 든 머그컵)을 보내며 "내가 그린 거라 그냥 써도 된다"고 확인해 줘서(저작권 걱정 없이 원본 그대로 사용), 배지 그림의 "나무 재활용" 문제를 줄이는 데 바로 썼습니다 — 지금까지 `week10`(바람 부는 숲)·`month20`(풍성한 달)·`months3`(계절을 건너)·`fav1`(보물 나무)·`fav5`(보물 다섯) 다섯 배지가 전부 나무 조각(`tree-*`)을 재사용하고 있어서 "나무 배지 말고 다양하게"라는 지적이 정확했습니다.

- **배경 제거**: 9장 전부 크림색 종이 배경이라 `scripts/knockout-bg.py --trim`으로 투명 PNG로 걷어냈습니다(원본은 `docs/illustrations/user-badges-src/`에 보관). 자동차·캠퍼·텐트처럼 원본이 컸던 파일은 최대 480px로 축소해 저장 용량을 줄였습니다(`public/illustrations/`).
- **매핑**: 나무·별·등불 재사용을 없애고 그림 하나당 배지 하나로 — `fav1`(보물 나무) → 물망초, `fav5`(보물 다섯) → 할미꽃(더 짙고 풍성한 다발로 상위 등급 표현), `week10`(바람 부는 숲, 한 주 10권) → 버섯 무리(비 온 뒤 하룻밤 새 무더기로 자라는 것 같은 느낌), `month20`(풍성한 달) → 도토리(가을 수확의 풍성함), `months3`(계절을 건너) → 여우(계절을 건너는 이미지), `week`(밤길 지킴이, 일주일 연속) → 티백 머그(매일 밤 따뜻한 차 한 잔). `rec10`/`rec30`/`rec100`(등불 따라 열/서른/백 걸음 — 추천도서 완독 수)은 걸음이 늘수록 더 멀리 갈 수 있는 탈것 진행으로 텐트 → 캠핑 트레일러 → 랜드로버 순으로 배정했습니다.
- **`components/illustration.tsx`**: `ILLUSTRATIONS` 표에 9개 원본 픽셀 크기를 등록(`forget-me-not`/`pasque-flower`/`mushroom-cluster`/`acorn`/`fox`/`tent`/`camper`/`land-rover`/`tea-mug`).
- 임시 미리보기 라우트(확인 후 삭제)로 9종 전부 딴 상태(컬러)·못 딴 상태(흑백·반투명) 두 가지를 렌더링해 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. 우리 숲(`lib/forest-scene.ts`의 배지→장식 매핑)에는 이 9종을 새로 안 얹었습니다 — week/week10/month20/months3/fav1/fav5는 나무가 아닌 다른 개수(count 없음)인 배지 목록 전용 배지고, 우리 숲 장식은 `ORNAMENT_BY_BADGE`에 등록된 것만 자리를 얻으므로 자연히 배지 목록 화면에서만 보입니다(숲 장식까지 늘리는 건 이번 요청 범위 밖이라 그대로 유지). DB 변경 없음.

## 그룹 가입 배지("첫 숲지기"·"숲지기 셋")의 곰 아이콘도 별로 교체 (사용자 지적: "곰은 그룹 가입하면 어차피 생기는거 아냐?")

바로 앞 라운드에서 곰·백로를 "사진·목소리·이야기꾼" 배지에서만 뺐는데, `group1`("첫 숲지기")·`group3`("숲지기 셋") 배지는 그대로 곰(`bear-lantern`) 아이콘을 쓰고 있었습니다. 사용자 지적이 정확합니다 — 그룹에 가입하면 숲길 끝에 실제 숲지기(곰·백로, 그룹마다 한 명, 고른 얼굴 그대로)가 이미 자동으로 서고, 우리 숲 장식(`ORNAMENT_BY_BADGE`)도 이 두 배지를 딸 때마다 곰을 나무 사이에 또 심고 있었어서 — 배지 카드 자체까지 곰이면 "그룹 가입 = 곰"이라는 같은 정보를 세 군데(숲지기 아바타·숲 장식·배지 카드)에서 반복하고 있었습니다.

- **`components/badge-art.tsx`**: `group1`→별 1개, `group3`→별 3개(가운데 큰 별 + 양옆 작은 별, 개수로 등급감)로 교체.
- **`lib/forest-scene.ts`**: `ORNAMENT_BY_BADGE`의 `group1`/`group3` 매핑도 `bear`→`star`로 맞춰 바꿨습니다(배지-장식 매핑은 항상 짝을 맞춰야 한다는 이 프로젝트의 기존 규칙). 이제 그룹 가입은 **숲길 끝의 진짜 숲지기 아바타 하나만**으로 표시되고, 배지·숲 장식에는 더 이상 곰이 중복으로 나오지 않습니다.
- 임시 미리보기 라우트(확인 후 삭제)로 두 배지의 새 별 아이콘을 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 배지 나무와 우리 숲 나무를 하나의 함수로 통일 + 우리 숲 높이 캡 + 전체 보기 모달 (사용자 지적: "배지상의 나무와 그림의 나무가 다르네", "화면 스크롤할정도로 커지면 안되니까")

우리 숲이 예뻐졌다는 칭찬과 함께 두 가지 구체적인 버그 신고를 받았습니다(스크린샷에 분홍색 손글씨로 "새싹 다섯" 배지 카드의 나무와 실제 숲 장면의 나무가 다른 모양이라고 표시).

- **원인**: 권수 마일스톤 나무를 그리는 함수가 **서로 다른 시기에 따로 만들어져 둘**이었습니다 — `components/badge-art.tsx`의 로컬 `milestoneArt(count)`(8단계 구간, 구간마다 나무·별을 1~5개 조합해 "빽빽해지는 숲" 느낌을 내던 예전 방식)와 `lib/forest-items.ts`의 `milestoneTreeArt(count)`(6단계 구간, 실제 우리 숲 장면이 쓰는 "마일스톤 하나 = 나무 한 그루" 함수). 두 함수의 구간 경계가 달라서(예: count=5는 `milestoneArt`에서 `<10`이라 `tree-bushy`, `milestoneTreeArt`에서도 `<10`이라 `tree-light` — 이름은 같아도 실제로 다른 나무 그림), 같은 배지인데 배지 카드와 숲 장면에서 다른 나무로 보이는 게 근본 원인이었습니다.
- **수정**: `badge-art.tsx`의 로컬 `milestoneArt` 함수를 완전히 지우고, `lib/forest-items.ts`의 `milestoneTreeArt`를 그대로 import해서 씁니다(`const pieces = count !== undefined ? [milestoneTreeArt(count)] : (ART[id] ?? [T("star", 20)]);`). 이제 배지 목록과 우리 숲 둘 다 **정확히 같은 함수 한 곳**에서 나무를 정하므로, 같은 권수 배지는 항상 같은 나무로 보입니다(구조적으로 다시 어긋날 수 없음).
- **우리 숲 높이 캡 + 전체 보기 모달(`components/forest-view.tsx`)**: 배지가 쌓일수록 나무·장식이 늘어나 숲 장면 자체가 메인 화면(오늘 탭이 아니라 `/forest`)을 스크롤하게 만들 수 있다는 지적을 반영했습니다. `sceneRef`로 장면 컨테이너의 실제 `scrollHeight`를 측정해(마운트 시 + 배지 변경 시) 300px(`FOREST_CAP_HEIGHT`)를 넘으면 `overflowing` 상태가 켜지고, 그 상태에서만 컨테이너에 `max-height`가 걸리고 아래쪽에 배경색과 자연스럽게 이어지는 그라데이션 페이드 + "숲 전체 보기 ›" 버튼이 뜹니다. 누르면 하단에서 올라오는 모달(`fixed inset-0`, 반투명 검은 배경 + 흰 시트)이 열려 같은 장면을 **캡 없이** 전부 보여주고, 시트 안에서 스크롤할 수 있습니다(모달이 떠 있는 동안 `document.body.style.overflow = "hidden"`으로 배경 스크롤도 잠급니다). 닫기는 이모지 "✕" 대신 손으로 그린 stroke 기반 SVG X 아이콘(디자인 원칙의 "이모지 금지" 준수, `components/child-group-bar.tsx`의 체크 아이콘과 같은 관례)이고, ESC 키로도 닫힙니다.
- **중복 없이 공유**: 장면 그리기(별 배경 + 나무/장식 그리드 + 아바타·숲지기)와 "선택한 배지 안내 카드"를 각각 `ForestScene`/`InfoCard` 내부 컴포넌트로 뽑아서, 캡된 미리보기와 모달 양쪽이 완전히 같은 JSX를 재사용합니다(로직이 두 군데로 갈라져 또 어긋나는 걸 방지).
- **오늘 탭 미리보기(`forest-strip.tsx`)는 그대로**: 거기는 이미 `MAX_ITEMS=10` + "우리 숲 보기 ›" 링크로 캡·이어보기 패턴이 있어서 이번 변경 대상이 아닙니다.
- 임시 미리보기 라우트(배지를 다량 확보하도록 1000일치 기록으로 구성, 확인 후 삭제)로 (1) 8개 마일스톤일 때는 캡이 안 걸리는 것, (2) 38개 마일스톤 전부 달성 시 캡이 걸리고 "숲 전체 보기" 버튼이 뜨는 것, (3) 눌렀을 때 모달이 전체 장면을 보여주는 것, (4) X 버튼으로 닫히는 것을 Playwright로 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## "검색에 안 나오는 책이에요" 문구가 검색 실패로 오해될 수 있음 (사용자 지적, 스크린샷)

숲지기의 책 찾기(`BookFinder`, 추천도서 올리기·숙제 만들기가 공유) 화면에서 제목 검색창 아래 항상 떠 있는 수동 입력 진입 버튼이 "검색에 안 나오는 책이에요"였는데, 사용자가 이 문구를 "검색 결과가 없다"는 뜻으로 오해할 수 있다고 짚었습니다 — 실제로는 검색 여부·결과와 무관하게 항상 보이는, "전권 세트처럼 애초에 검색이 안 되는 책은 직접 입력하라"는 안내용 버튼입니다.

- **`components/book-finder.tsx`**: 문구를 **"+ 찾는 책이 없나요? 직접 입력하기"**로 바꿔, 검색 결과 유무를 진술하는 문장이 아니라 눌러야 하는 행동(직접 입력하기)임이 분명하게 드러나도록 했습니다.
- **`README.md`**: 같은 문구를 인용하던 테스트 가이드 문장도 함께 갱신했습니다.
- DB 변경 없음. `npm run lint`/`rm -rf .next && npm run build` 통과.

## "책 추가" 계열 버튼 색을 "책 기록하기"와 통일 (사용자 요청, 스크린샷에 손그림 표시)

오늘 탭의 "+ 책 기록하기"(짙은 숲 초록 `--point-deep`)와 책장 탭의 "+ 책"이 서로 다른 초록(`--point`, 더 밝은 청록빛)을 쓰고 있어서 미묘하게 안 맞아 보였고, 하단 탭의 "추가"는 아예 다른 색 계열(연두 `--sprout`)이었습니다. 사용자가 스크린샷에 두 화면의 버튼을 손그림으로 짚으며 "책 추가 버튼들은 전부 오늘에 있는 책 기록하기 버튼의 녹색을 따라 해줘"라고 요청해, "책을 추가/기록하는" 기능을 가진 버튼 전부를 `--point-deep`로 통일했습니다.

- **`components/library-shelf.tsx`/`app/library/page.tsx`**: 책장 탭 검색줄의 "+ 책" 버튼과, 책장이 비었을 때 뜨는 빈 상태의 "+ 책" 버튼 둘 다 배경을 `var(--point)` → `var(--point-deep)`로 바꿨습니다.
- **`components/bottom-nav.tsx`**: 하단 탭 "추가"의 색 계열을 연두(`--sprout`/`--sprout-deep`)에서 초록(`--point`/`--point-deep`)으로 바꿨습니다 — 예전엔 다른 탭과 구분되도록 일부러 다른 색 계열(사용자가 이전에 "다른 색상으로 지정해줘 연두색이나"로 요청)을 썼지만, 이번엔 "책 추가 버튼은 전부 같은 초록으로"라는 더 최신 요청을 우선했습니다. "추가" 탭이 항상 초록 알약 배경으로 떠 있는 동작(다른 탭은 활성일 때만 배경이 켜짐)은 그대로 유지하고, 색상 값만 `rgba(47,168,79,…)`(point 계열)로 바꿨습니다. 내부적으로 `accent: "sprout"` 필드명도 `accent: "highlight"`로 정리했습니다(색 자체가 sprout이 아니게 됐으므로).
- 임시 미리보기 라우트(확인 후 삭제)로 세 버튼(오늘 탭 "+ 책 기록하기", 책장 탭 "+ 책", 하단 탭 "추가"·"추가(활성)")을 나란히 렌더링해 같은 초록 계열로 보이는 걸 스크린샷으로 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 숲지기 책 추가 버튼 강조 + 숙제/추천도서 탭 대비 강화 + 추천도서 목록형 추가 (사용자 요청, 스크린샷 2장에 손그림 표시)

두 화면을 손그림으로 짚은 요청 세 가지를 반영했습니다: "1. 숲지기가 책추가하는 버튼도 좀 더 잘보이고 같은 녹색으로! 2. 그리고 숙제와 추천도서 좀 더 잘보이게. 그리고 추천도서 책장 목록형으로도 볼 수 있게해줘"

- **숲지기 "+ 책 추가" 버튼**: `components/operator-book-browser.tsx`(`/teacher/books`)의 컨트롤 줄에 있던 "+ 책 추가"가 흰 배경 + 테두리만 있는 아웃라인 버튼이라 눈에 잘 안 띄었습니다. 배경을 `var(--point-deep)` 채움 + 흰 글자로 바꿔, 방금 통일한 다른 "책 추가" 버튼들(책장 탭 "+ 책", 오늘 탭 "+ 책 기록하기")과 같은 짙은 초록으로 맞췄습니다. 옆의 "내보내기" 버튼은 보조 액션이라 아웃라인 스타일 그대로 뒀습니다.
- **"숙제"/"추천도서" 소제목 탭 대비 강화**: `components/child-group-bar.tsx`(`/group` 상단 고정 바)의 밑줄형 탭 — 선택 안 된 쪽 글자가 옅은 회색(`--ink-2`)이라 "탭 두 개가 잘 안 보인다"는 지적(양쪽 라벨 모두 노란 형광펜으로 표시)을 반영해, 옅은 초록 트랙(`--sprout-pale`) 위에 선택된 탭만 짙은 초록(`--point-deep`) 알약으로 채우고 선택 안 된 탭도 진한 글자(`--ink`, 굵게 600)로 바꾼 세그먼트 컨트롤로 다시 짰습니다 — 이제 어느 쪽을 보고 있어도 두 라벨이 뚜렷이 읽힙니다. 고정 바 높이가 조금 늘어나(세그먼트 컨트롤의 여백이 예전 밑줄 탭보다 큼) `lib/group-bar-height.ts`의 `CHILD_GROUP_BAR_HEIGHT`를 140 → 148로 올렸습니다(본문이 고정 바 뒤에 가려지는 사고가 전에 한 번 있었어서 여유 있게).
- **추천도서 목록형 보기 추가**: `components/recommend-shelf.tsx`(부모가 보는 숲길의 추천도서, 지금까지 표지 선반 하나만 있었음)에 책장 탭·숲지기 추천도서 탭과 같은 "보기 방식" 드롭다운(전면/목록)을 추가했습니다. 목록 보기는 책장 탭의 기존 목록 줄 패턴(44×32 작은 표지 + 제목 + 작가, 흰 박스 안 `border-top` 구분선)을 그대로 재사용하고, 오른쪽에 책갈피 토글(`ShelfBookmark`)만 둡니다 — 추천도서는 완료를 요구하지 않는다는 기존 원칙 그대로, 목록 보기에도 읽음 체크는 넣지 않았습니다. 마지막 보기 모드는 `localStorage`(`chaeksup:recommend-shelf-view`)에 저장해 다음 방문에도 유지됩니다.
- 임시 미리보기 라우트(`OperatorBookBrowser`·`RecommendShelf`를 가짜 책 3권으로 렌더링, 확인 후 삭제)로 필 버튼과 전면/목록 전환을 스크린샷으로 확인했고, `ChildGroupBar`는 실제 컴포넌트가 Supabase 세션을 요구해 정적 HTML 복제본으로 세그먼트 컨트롤 모양과 높이를 가늠했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 하단 탭 "추가" 아이콘을 짙은 초록 채움 동그라미로 (사용자 지적, 스크린샷에 손그림: "가운데 추가 버튼도 다른 책추가버튼같은 녹색으로")

바로 앞 라운드에서 "추가" 탭의 색값 자체는 이미 `--point-deep` 계열(rgba(47,168,79,…))로 맞춰 놨었지만, 실제 렌더링은 옅은 초록 배경 위에 **얇은 스트로크 원+십자** 아이콘(`AddIcon`, `fill="none"`)이라 "+  책"·"+ 책 기록하기" 같은 **짙은 초록으로 꽉 채운** 버튼들과 무게감이 달라 보였습니다(스크린샷에서 두 버튼을 손그림으로 짚어 비교).

- **`components/bottom-nav.tsx`**: "추가" 탭(`isHighlighted`)일 때만 스트로크 아이콘 대신, `var(--point-deep)`로 꽉 채운 26px 동그라미 안에 흰색 십자 SVG를 그리도록 바꿨습니다 — 다른 "책 추가" 버튼들과 정확히 같은 색을 흰 글자/아이콘 대비로 재현합니다. 나머지 탭(오늘·책장·그룹·프로필)의 스트로크 아이콘 렌더링은 그대로입니다.
- 정적 HTML 복제본(`+ 책`, `+ 책 기록하기` 버튼과 하단 알약 바를 나란히 배치)을 Playwright로 렌더링해 세 요소가 같은 짙은 초록으로 보이는 걸 스크린샷으로 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## "추가" 탭 배경 자체를 짙은 초록으로 (사용자 지적: "그냥 버튼 자체를 연한 녹색말고 짙은 초록으로... 눌렀을땐 뭔가 다른 색이여야겠지?")

바로 앞 라운드는 탭 안의 작은 아이콘 동그라미만 짙은 초록으로 채웠는데, 정작 그 아이콘을 감싸는 **탭 알약 배경 자체**는 여전히 옅은 반투명 초록(`rgba(47,168,79,0.16)` 평소 / `0.32` 활성)이라, 사용자 눈에는 "버튼 자체가 연한 녹색"으로 계속 보였습니다. 이번엔 탭 배경 자체를 바꿨습니다.

- **`app/globals.css`**: `--point-deepest: #123D26`(신규) — `--point-deep`(#1B5E3A)보다 한 단계 더 짙은 색으로, 이미 항상 채워져 있는 초록 배경 위에서 "지금 이 화면에 있음(활성)"을 표시할 때만 씁니다. 더 이상 안 맞는 "새싹 연두 — 하단 탭 '추가' 버튼 전용" 주석도 정리했습니다(그 색은 이전 라운드에서 이미 `--point-deep` 계열로 통일됨).
- **`components/bottom-nav.tsx`**: "추가" 탭의 배경을 평소엔 `var(--point-deep)`(꽉 채움), 그 화면(`/library/add`)에 있을 때는 `var(--point-deepest)`(더 짙은 색)로 바꿨습니다. 배경이 이미 짙은 초록으로 꽉 찼으므로 안에 있던 별도의 26px 동그라미(아이콘 전용 배경)는 필요 없어져 없앴고, 흰색 십자 SVG만 배경 위에 바로 그립니다. 글자·아이콘 색도 이제 흰색(`#fff`)입니다(짙은 배경 위에 옅은 초록 글자는 안 보여서).
- 정적 HTML로 평소/활성 두 상태를 나란히 렌더링해 활성 상태가 뚜렷이 더 짙은 색으로 구분되는 걸 Playwright 스크린샷으로 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 전체 점검 + 실사용 출시 준비 체크 (사용자 요청: "데이터 이름·디자인 마감·내용 전부 점검하고 정리, 그다음 실제 어플 만드는 과정")

"전체 점검"이 정확히 몇 번째인지 셀 수 없을 만큼 이 세션 내내 반복해 온 라운드라(위 로그 전체가 그 기록입니다), 이번엔 아직 안 잡힌 게 남아 있는지 기계적으로 다시 훑었습니다 — 화면에 남은 옛 탭 이름 문구(더보기 탭·숲길 탭·추천 탭 등, grep으로 재확인 — 남아있는 건 전부 코드 주석뿐이고 사용자에게 보이는 문구는 이미 이전 라운드에서 다 정리돼 있었습니다), 안 쓰이는 컴포넌트 파일(전부 어딘가에서 import됨, 고아 파일 없음), TODO/FIXME 마커(0건), `npm install` 의존성(three.js 등 전부 실제로 쓰임) 확인했고 새로 발견된 문제는 없었습니다.

- **README의 마이그레이션 안내가 0005에서 멈춰 있던 걸 발견해 고쳤습니다**: "CLI를 안 쓰면 SQL Editor에서 0001~0005만 순서대로 실행하라"는 안내가 그대로 남아 있어서, 그대로 따라가면 마이그레이션 0006~0028(숙제 답변 저장·상태 관리·프로필 통합·숲지기 이름 등 이 세션 후반부 기능 전부)이 누락됩니다. "0001부터 번호 순서대로 전부(현재 0028까지)"로 바꿨습니다.
- **"실제 어플 만드는 과정"을 사용자에게 확인 결과 "실제 배포/출시 준비"로 확정**: 코드로 손댈 수 있는 부분만 우선 처리했습니다 — `next.config.ts`에 최소 보안 헤더(`X-Frame-Options: DENY`로 클릭재킹 차단, `X-Content-Type-Options: nosniff`, `Referrer-Policy`, 카메라·위치 권한을 기본 차단하는 `Permissions-Policy` — 아이 사진·음성이 오가는 서비스라 이 정도는 배포 전 최소선이라고 판단) 추가. CSP(콘텐츠 보안 정책)는 카카오 CDN 표지·구글 폰트 등 허용해야 할 외부 출처가 여러 곳이라 잘못 좁히면 화면이 깨질 위험이 커서 이번 범위에서는 넣지 않았습니다.
- **코드로 할 수 없어 사용자가 직접 확인·처리해야 하는 것(Supabase 서비스 롤 키·Vercel 대시보드 접근 권한이 이 세션에 없어서)**:
  1. **마이그레이션 0016~0028이 실제 Supabase 프로젝트에 전부 적용됐는지** — CLAUDE.md 곳곳에 "SQL Editor에서 실행 필요"로 표시된 것들이 밀려 있으면 그 기능들이 "불러오지 못했어요"로 보입니다.
  2. **Supabase 요금제**: 지금 무료 플랜이면 실사용자가 늘 때 콜드 스타트·커넥션 제한·(이미 한 번 겪은) DB 정지 위험이 있어 Pro 전환을 미리 검토해 두는 게 좋습니다.
  3. **Vercel 프로젝트 설정**: 커스텀 도메인 연결, 환경변수(`KAKAO_REST_API_KEY` 등 `.env.example`의 항목 전부)가 프로덕션 환경에 등록됐는지, 리전이 서울(`icn1`, `vercel.json`에 이미 고정)인지.
  4. **Supabase Auth Redirect URLs**: 배포 도메인의 `/auth/confirm`이 등록돼 있어야 비밀번호 재설정이 실제로 동작합니다(README에 이미 안내됨).
  5. **PWA 설치 확인**: 실제 배포 도메인에서 "홈 화면에 추가"가 아이폰·안드로이드 양쪽에서 정상 동작하는지(매니페스트·아이콘은 이미 갖춰져 있음, `public/manifest.webmanifest`).
  6. **오류·사용량 모니터링**: 지금은 별도 에러 트래킹(Sentry 등)이 없고 `app/error.tsx`(화면 안내)만 있습니다 — 실사용자가 생기면 실제로 어디서 얼마나 오류가 나는지 알 방법이 없다는 뜻이라, 필요하면 이후 라운드에서 붙일 수 있습니다(비용이 드는 외부 서비스라 이번엔 추가하지 않았습니다).
  7. **네이티브 앱스토어 출시는 이 프로젝트 범위 밖입니다** — 지금은 PWA(웹 앱)이고 CLAUDE.md 기술 스택에도 네이티브 래핑(예: Capacitor) 계획이 없습니다. 앱스토어 등록까지 원하시면 그건 완전히 별도 작업(네이티브 래퍼 추가, 스토어 심사 자산 준비 등)이라 먼저 범위를 정해야 합니다.
- `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 그룹 탭 — 타일 줄과 숙제/추천도서 탭 사이 간격 + 그룹명을 흰 박스 밖으로 (사용자 스크린샷 2장, 손그림 표시)

그룹 탭 상단 고정 바(`ChildGroupBar`)와 그 아래 추천도서 화면 두 가지를 지적받았습니다.

- **타일 줄과 숙제/추천도서 세그먼트 탭이 너무 붙어 있음**: `components/child-group-bar.tsx`의 그룹 타일 줄(`pt-2.5`만 있고 아래 여백이 없음)과 바로 아래 세그먼트 탭 줄이 맞닿아 있었습니다. 타일 줄에 `pb-3`(12px)를 추가해 둘 사이에 눈에 보이는 간격을 줬습니다. 고정 바 전체 높이가 그만큼 늘어나서, `lib/group-bar-height.ts`의 `CHILD_GROUP_BAR_HEIGHT`를 148 → 152로 다시 실측해 올렸습니다(정적 HTML을 360px에서 Playwright로 렌더해 새 높이 131px 확인 후 여유를 더함).
- **"{그룹명}의 추천도서" 제목이 흰 박스(카드) 안에 갇혀 있었음**: `app/group/page.tsx`의 추천도서 탭이 `Section`(카드 머리에 제목을 넣는 컴포넌트)으로 `RecommendShelf`를 감싸고 있어서, 그룹명이 카드 헤더 안에 들어가 있었습니다. 사용자는 이 제목이 카드 밖으로 나와야 한다고 지적했는데, 실제로 이미 같은 화면의 숙제 탭(`AssignmentsBrowser`)이나 `/group/past` 같은 화면들은 전부 배경 위에 바로 놓인 `<h1>` 제목(박스 없음)을 쓰고 있어서 — 추천도서 탭만 예외적으로 박스에 갇혀 있던 것이었습니다. `Section` 대신 다른 화면들과 같은 `<h1 className="d text-xl">{그룹명}의 추천도서</h1>` 제목 + 그 아래 박스 없는 `RecommendShelf`로 바꿔서 일관성을 맞췄습니다(`RecommendShelf` 자체는 이미 분야 칩·권수 줄을 박스 없이 그리고 있어서 카드로 감쌀 필요가 없었습니다). 안 쓰게 된 `Section` import를 지웠습니다.
- `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 그룹 탭 추천도서 — 제목은 밖, 내용은 다시 흰 카드 안으로 (사용자 지적: "아이 내용은 흰 카드 안에 둬야지 너무 정신 사납자나")

바로 전 라운드에서 "그룹명이 흰 박스가 아니라 밖에 따로 나와야 한다"는 지적에 `Section`을 걷어내고 분야 칩·권수 줄·책 선반 전부를 배경 위에 바로 그리도록 바꿨는데, 이번엔 그 결과 화면(같은 스크린샷 위치, 24권 그리드 부분)이 흰 카드 하나 없이 배경 위에 칩·숫자·책 표지가 그대로 떠 있어 "정신 사납다"는 반대 방향의 지적을 받았습니다. 두 요구를 동시에 만족시키려면 "제목(그룹명)은 밖, 그 아래 실제 내용은 카드 안"으로 나눠야 합니다 — 실제로 이 조합은 이미 `/recommend/[groupId]`(그룹 상세 화면)에서 쓰이고 있었습니다: 페이지 상단에 그룹 이름을 박스 없이 보여준 뒤, 그 아래 `Section title="추천도서"`(그룹명이 아니라 일반 라벨)로 `RecommendShelf`를 감쌉니다.

- **`app/group/page.tsx`**: 다시 `Section` import를 추가하고, `<h1>{그룹명}의 추천도서</h1>`(박스 없음, 그대로 유지)는 남긴 채 그 아래 `RecommendShelf`를 `<Section title="추천도서">`로 감쌌습니다 — 카드 헤더에 들어가는 제목은 그룹명이 아니라 `/recommend/[groupId]`와 똑같은 일반 라벨("추천도서")이라, "그룹명은 박스 밖"이라는 지난 지적과 "내용은 박스 안"이라는 이번 지적이 동시에 지켜집니다.
- **일관성 확인**: 이 화면의 숙제 탭(`AssignmentsBrowser`)은 원래도 자체적으로 카드 구조(지금 해야 할 숙제 등 섹션별 박스)를 갖고 있어 이번 변경 대상이 아닙니다. `RecommendShelf`의 list 보기 모드는 자체적으로도 흰 박스를 그리는데, `/recommend/[groupId]`에서도 같은 이중 박스 구조가 이미 쓰이고 있어 새로 생긴 문제는 아닙니다.
- `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 책장 탭도 흰 카드 안으로 (사용자 지적: "기본 책장도 흰 카드 안으로 넣어줘")

바로 앞 항목(그룹 탭 추천도서)과 같은 지적이 책장 탭에도 그대로 적용됐습니다 — 검색창·"+ 책"·⋯ 메뉴 줄은 이미 각자 흰 테두리 상자였지만, 그 아래 "N권 ── 모든 상태 ▾ 최신순 ▾" 필터 줄과 나무 선반 위 책 그리드(전면·책등·목록 세 보기 전부)가 배경 위에 그대로 떠 있었습니다.

- **`components/library-shelf.tsx`**: 검색·+책·⋯ 메뉴 줄 아래에 `rounded-[var(--r)] border` 흰 카드(`var(--card)` 배경, `mt-[12px] p-4`) 하나를 새로 두르고, 그 안에 권수·필터 드롭다운 줄부터 "검색 결과가 없어요" 안내, 전면/책등/목록 세 보기 그리드까지 전부 넣었습니다. `rounded-[var(--r)]` 클래스라 다른 카드들과 같은 은은한 그림자가 자동으로 붙습니다(`app/globals.css`의 공통 규칙). 책장 정리 패널(⋯ 메뉴에서 켜는 것)은 이미 자기 카드(초록 테두리로 강조)가 있어 이번 변경 대상에서 뺐고, 검색·+책 줄 바로 아래·새 카드 위에 그대로 남았습니다.
- **목록 보기의 이중 박스**: `mode="list"`는 원래도 달별로 자체 흰 박스(구분선 목록)를 그리는데, 이제 그 바깥에 카드가 하나 더 생겨 이중 박스가 됩니다 — 바로 앞 항목의 `RecommendShelf`/`/recommend/[groupId]`와 같은 이유(둘 다 같은 `var(--card)` 배경이라 테두리 선 하나만 겹치는 정도)로 새로운 문제가 아니라고 판단해 그대로 뒀습니다.
- `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 그룹 탭 타이포그래피·여백 정돈 — 글꼴은 그대로, 크기·굵기·줄간격·여백만 (사용자 요청)

"그룹 탭 화면이 넘 복잡해서" — 현재 쓰는 글꼴과 영역별 글꼴 배치(`.d`/`.hand`/기본 시스템 폰트 구분)는 그대로 유지하고, **글자 크기·굵기·줄간격·주변 여백**만 조정해서 정보 위계와 정렬을 개선해 달라는 요청이었습니다. 사용자가 참고로 첨부한 "예상 시안" 이미지는 화면 구성·배치(느낌)만 참고하고 실제 글꼴·검색창·아이콘 토글·밑줄 탭 같은 UI는 그대로 두라고 명시했습니다 — 실제로 그 시안은 지금 앱에 없는 검색창, `<select>` 대신 아이콘 토글, 알약형 대신 밑줄형 세그먼트 탭을 보여주는 목업이라(코드 대조로 확인), 문구·기능·상호작용 방식은 전혀 건드리지 않고 오직 타이포그래피·여백만 시안의 "여유로운 느낌"에 맞춰 조정했습니다. 특히 밑줄형 탭은 예전에 "선택 안 된 쪽 글자가 옅어서 탭 자체가 잘 안 보인다"는 지적으로 지금의 채워진 알약 세그먼트 컨트롤로 일부러 바꾼 이력이 있어(위 `child-group-bar.tsx` 관련 항목 참고) 되돌리지 않았습니다.

- **`components/recommend-shelf.tsx`(그룹 탭의 추천도서 내용, 시안이 보여준 바로 그 화면)**: 분야 칩(`전체·한글·수학…`)의 글자를 `text-xs`→`text-sm`, 안쪽 여백을 `px-3 py-1`→`px-3.5 py-1.5`, 칩 사이 간격을 `gap-1.5`→`gap-2`로 키워 시안처럼 여유 있게 눌리는 크기로 키웠습니다. 그 아래 "N권 · 읽은 책 M권" 줄도 `text-xs`→`text-sm`(위 여백 `mt-2`→`mt-3`), 보기 방식 `<select>`는 `text-[11px]`→`text-xs`로 살짝 키웠습니다(상호작용 방식 자체는 그대로). 칩 줄→권수 줄→책 그리드 사이 간격을 전부 한 단계씩 넓혔고(그리드 `mt-4`→`mt-5`, 선반 행 사이 `gap-3`→`gap-5`, 선반 안 제목 블록 `mt-1.5`→`mt-2`), **책 제목**을 전면 보기·목록 보기 모두 `text-xs`(13.5px)→`text-sm`(15.75px)로 키우고 작가명 위에 `mt-0.5` 간격을 추가해 제목과 겹쳐 보이지 않게 했습니다 — "아이가 읽는 책 제목은 충분히 잘 보이게" 조건을 더 확실히 지킵니다. 작가명도 전면 10px→11px, 목록 11px→12px(`text-xs`)로 한 단계씩.
- **`app/group/page.tsx`**: 추천도서 탭의 `<h1>{그룹명}의 추천도서</h1>`(박스 밖 제목, 그대로 유지)와 그 아래 `Section`(흰 카드) 사이 간격을 `mt-4`→`mt-5`로 맞춰, CLAUDE.md에 이미 정해둔 "섹션 사이 간격은 mt-5로 통일" 규칙과 일치시켰습니다.
- **`components/child-group-bar.tsx`(그룹 탭 상단 고정 바)**: 그룹 타일 줄과 그 아래 "숙제/추천도서" 세그먼트 탭 줄이 서로 바짝 붙어 있어 화면 맨 위가 빽빽해 보이던 걸, 세그먼트 탭 줄에 `pt-1.5`를 추가해 두 줄 사이에 눈에 보이는 숨 쉴 틈을 줬습니다(글꼴·탭 모양·상호작용은 전혀 안 바꿈). 바 높이가 늘어난 만큼 `lib/group-bar-height.ts`의 `CHILD_GROUP_BAR_HEIGHT`를 152→156으로 다시 실측해 올렸습니다(정적 HTML 복제본을 360px에서 Playwright로 렌더해 152px 확인 후 폰트 렌더링 오차 감안한 여유를 더함 — 본문이 고정 바 뒤에 숨는 사고가 과거에 있었던 자리라 항상 여유 있게 잡습니다).
- **숙제 탭(`AssignmentToday`/`AssignmentsBrowser`)은 이번엔 손대지 않았습니다**: 바로 앞선 "숙제 카드 전면 개편" 라운드에서 정보 순서(마감일→등록일→제목/설명→책 목록→질문)와 크기·굵기·여백을 이미 사용자 스펙대로 정밀하게 맞춰 놓은 상태라, 이번 지적(첨부 시안)이 가리킨 화면도 추천도서 쪽이었고 숙제 카드에서 새로 지적된 문제는 없어 재조정하지 않았습니다.
- 정적 HTML(Playwright, 360px)로 칩·권수줄·책 제목이 커진 뒤에도 카드 폭 안에 자연스럽게 들어가는 걸 확인했습니다(임시 파일, 확인 후 삭제). `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 책장·추천도서 보기 전환을 드롭다운 대신 아이콘 두 개로 (사용자 요청 + 스크린샷 손그림: "이 동그라미친 영역처럼 좀 덜복잡해보이게")

"책장에도 추천도서에도 전면 보기/목록형보기는 이런식으로 심플하고 간단하게 표시해줘"라며 참고 시안 스크린샷(그룹 탭 추천도서 화면)의 "24권 · 읽은 책 2권" 옆에 붙은 격자/줄 아이콘 두 개를 가리켰습니다. 이어서 같은 시안에 훨씬 넓은 원(상단 그룹 타일부터 분야 칩·권수 줄까지)을 다시 그려 "이 동그라미친 영역처럼 좀 덜 복잡해 보이게"라고 재차 강조했는데, 그 영역 안의 검색창·타일·탭은 이전 라운드에서 이미 의도적으로 지금 모양으로 확정된 것들이라(밑줄 탭 대신 채워진 세그먼트 컨트롤 등) 다시 건드리지 않았고, 실질적으로 시안과 달랐던 부분 — 보기 전환 드롭다운 — 을 시안처럼 아이콘 두 개로 바꾸는 데 집중했습니다.

- **`components/recommend-shelf.tsx`**: "전면 보기"/"목록 보기" `<select>`를 없애고, 같은 자리에 격자 아이콘(`CoverViewIcon`)·줄 아이콘(`ListViewIcon`) 두 개를 나란히 뒀습니다(28×28px 원형 버튼, 선택된 쪽만 옅은 초록 배경 + 짙은 초록 아이콘, 나머지는 회녹색). 동작(`switchMode`, `localStorage` 저장)은 그대로입니다.
- **`components/library-shelf.tsx`**: 책장은 보기 방식이 전면·책등·목록 셋이라 시안과 똑같이 둘만 아이콘으로 뺄 수는 없어서, **자주 쓰는 전면·목록 두 가지만** 아이콘 토글로 앞에 내놓고 **덜 쓰는 책등은 그대로 "⋯" 메뉴 안에** 남겼습니다. 처음엔 아이콘 토글을 "N권 ── 책장·그룹 ▾ 모든 상태 ▾ 최신순 ▾" 필터 줄 끝에 붙였는데, 정적 HTML로 360px에서 실측해보니 필터 세 개가 이미 그 줄 폭을 꽉 채우고 있어서(그룹·이름표가 모두 있는 경우 최대 폭 381px vs 가용 286px) 아이콘이 옆으로 밀려 잘리거나 가로 스크롤이 생겼습니다 — 오히려 더 복잡해 보이는 역효과라 판단해, 대신 **검색창 줄**("검색창 + 책 + ⋯" 줄, 필터보다 여유가 많음)에 테두리 하나로 묶은 아이콘 페어를 뒀습니다(같은 360px 실측으로 검색창이 122px까지 줄어도 한 줄에 다 들어가는 것 확인, 바깥 여백도 `gap-2`→`gap-1.5`로 살짝 좁혀 여유를 더 냈습니다). "⋯" 메뉴에서는 이제 "전면 책장"/"읽은 순서 목록" 항목을 빼고 "책등 책장으로 보기" 하나만 남았습니다(그 아래 책장 정리·내보내기는 그대로).
- 정적 HTML(Playwright, 360px)로 두 화면의 새 아이콘 토글이 시안처럼 한 줄에 깔끔하게 들어가는 걸 스크린샷으로 확인했습니다(임시 파일, 확인 후 삭제). `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 시안대로 다시 — 보기 아이콘·중복 제목·그룹 바 흰 배경 (사용자 지적: "시안까지 넣어줘도 그걸 따라 못하냐")

바로 앞 라운드가 시안과 여러 군데 어긋나 있다는 지적을 스크린샷 네 장(현재 책장·현재 그룹 탭·시안 확대 크롭·시안 전체)에 X·형광펜으로 표시해 받았습니다. 이번엔 **정적 HTML 복제본이 아니라 임시 미리보기 라우트에서 실제 컴포넌트를 렌더링**해 시안과 나란히 비교하며 고쳤습니다(앞 라운드가 어긋난 이유가 손으로 쓴 복제본만 보고 판단한 탓이라, 확인 방식부터 바꿨습니다).

- **보기 아이콘을 시안 모양 그대로**(`components/icons/misc-icons.tsx`에 `GridViewIcon`·`ListRowsIcon` 추가, `components/view-toggle.tsx` 신규로 두 화면이 공용): 스트로크 테두리 아이콘 + 원형 배경 알약이었던 걸, **채워진 2×2 격자**와 **점+줄 목록** 아이콘으로 바꾸고 배경·테두리를 아예 없앴습니다. 켜진 쪽만 `--point`(초록), 꺼진 쪽은 `--ink-2` 45% — 시안 크롭과 같은 모습입니다.
- **책장 탭**: 검색줄에 붙여 놓았던 테두리 토글(사용자가 X 표시)을 빼고, 시안처럼 **"N권 ──────── [격자][목록]"** 줄로 옮겼습니다. 필터 드롭다운(책장·그룹/상태/정렬)까지 한 줄에 두면 셋 다 뜰 때 아이콘이 화면 밖(x=377 > 360)으로 밀려나는 걸 **실제 렌더링으로 측정해 확인**하고, 드롭다운은 그 아래 오른쪽 정렬 줄로 내렸습니다(이제 아이콘 오른쪽 끝 286px로 항상 보임).
- **그룹 탭의 중복 제목 제거**: 이미 "추천도서" 탭 안인데 제목("{그룹명}의 추천도서")과 카드 머리("추천도서")가 같은 말을 두 번 더 반복하고 있었습니다. `Section` 래퍼(머리글 포함)를 걷어내고 **머리글 없는 흰 카드**로 바꾼 뒤, 제목은 시안처럼 **그룹 이름만**(`7-8세 어린이`) 남겼습니다. 크기도 페이지 `text-xl`(22.5px)에서 **19px/26px**로 낮추고, 위(20px)·아래(16px) 여백을 비슷하게 맞춰 "쌩뚱맞게 크고 여백이 안 맞는다"는 지적을 해소했습니다.
- **그룹 바의 흰 배경 제거**(`components/child-group-bar.tsx`): 고정 바가 흰 패널(`--card` + 그림자 + 아래 테두리)이라 시안처럼 배경과 이어지지 않았습니다. 배경을 `--paper`(세이지)로 바꾸고 그림자·테두리를 없애 타일 줄과 숙제/추천도서 탭이 페이지 배경 위에 그대로 얹힌 것처럼 보이게 했습니다(고정 바라 배경은 불투명해야 해서 투명 대신 같은 색을 씁니다). 바가 세이지가 되면서 안 켜진 "전체" 타일이 묻히므로 그 배경만 `--sprout-pale`로 올렸습니다.
- **추천도서 목록 보기의 박스 안 박스 제거**: 이미 흰 카드 안인데 목록이 또 자기 테두리·배경을 그리고 있어서(좌우 여백도 16+16=32px로 어긋남) 테두리·배경을 빼고 줄 사이 구분선만 남겼습니다.
- 임시 미리보기 라우트(`app/preview-tmp`, 확인 후 삭제)에서 실제 `RecommendShelf`·`LibraryShelf`를 360/390px로 렌더해 전면·목록 두 상태와 필터 셋이 모두 뜨는 최악 폭까지 스크린샷으로 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## 책장의 "책등 보기"도 같은 아이콘 줄로 (사용자 요청: "책등보기도 전면보기나 목록형보기 아이콘처럼 심플하게 아이콘화")

앞 라운드에서 책등만 "⋯" 메뉴에 남겨 뒀는데, 나머지 둘과 같은 자리에 아이콘으로 놓아 달라는 요청을 받았습니다.

- **`components/icons/misc-icons.tsx`**: 격자·목록과 같은 채움 스타일의 **`SpineBarsIcon`**(선반 위에 세워 둔 책 세 권 + 바닥 선반 줄)을 추가했습니다. 이제 안 쓰게 된 예전 스트로크 아이콘 세 개(`CoverViewIcon`/`SpineViewIcon`/`ListViewIcon`)는 지웠습니다.
- **`components/view-toggle.tsx`**: `modes` prop을 받아 아이콘 개수를 정하도록 넓혔습니다 — 책장은 `["cover","spine","list"]`(세 개), 추천도서는 기본값 `["cover","list"]`(두 개, 책등 보기가 없는 화면이라). 타입상 추천도서 쪽 `switchMode`는 책등을 못 받으므로 호출부에서 좁혀 넘깁니다.
- **`components/library-shelf.tsx`**: "⋯" 메뉴에 있던 "책등 책장으로 보기" 항목을 없앴고(메뉴엔 이제 책장 정리·내보내기만), 권수 줄의 토글이 아이콘 세 개가 됐습니다. 메뉴 버튼의 `aria-label`도 "책장 정리와 내보내기"로 맞췄습니다.
- 임시 미리보기 라우트에서 실제 `LibraryShelf`를 390px로 렌더해 세 아이콘이 한 줄에 들어가고 책등 보기가 실제로 전환되는 것까지 스크린샷으로 확인했습니다(확인 후 삭제). `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.
- **숲지기 추천도서 탭(`components/operator-book-browser.tsx`)은 그대로 뒀습니다** — 거기도 같은 세 가지 보기를 `<select>`로 고르고 있어 나중에 같은 토글로 맞출 수 있지만, 이번 지적은 부모가 보는 책장 화면이라 범위를 넓히지 않았습니다.

## 숙제 카드 — 연두색 띠 대신 선과 정렬로 구분 (사용자 시안 + 스펙 5줄)

숙제 카드의 제목·설명 구역이 연한 초록 띠(`--paper`)로 칠해져 있던 걸, 사용자가 직접 만든 시안과 함께 다섯 줄 스펙으로 정리해 줬습니다: 카드 전체 흰색 / 숙제 설명은 왼쪽의 짧고 얇은 세로선으로 묶기 / 책 목록은 가로 구분선 + "읽을 책"으로 구분 / 진행 숫자는 설명 옆에서 책 목록 쪽으로 이동 / 마감일은 상단에 크게 유지. 글꼴은 기존 그대로.

- **`components/assignment-today.tsx`의 `AssignmentCard`**: 제목·설명 구역의 `background: var(--paper)`를 없애 **카드 전체가 흰색**이 됐고, 그 두 줄을 `border-l-2 pl-3`(색 `rgba(27,94,58,0.35)` — 흰 배경에서 읽히는 옅은 숲 초록)로 묶었습니다. 그 아래 **읽을 책** 구역은 좌우 20px 안쪽으로 들어간 가로 구분선(`mx-[20px]` + `border-top`)으로 나누고, 라벨 "읽을 책" 오른쪽에 **"N권 중 M권 읽었어요"**(예전엔 제목 옆에 있던 진행 숫자)를 뒀습니다. 마감일 줄(19px + 급함/완료 배지 + 오른쪽 등록일)은 그대로입니다.
- **질문·낭독도 같은 방식**: 책 목록과 똑같이 가로 구분선 + 라벨(미션 종류에 따라 "질문" / "낭독" / "질문 · 낭독")로 나눴습니다. 예전의 **"1단계 · 책 읽기" / "2단계 · 질문 확인 및 답변 작성" 문구는 뺐습니다** — "읽을 책"·"질문" 라벨과 위아래 순서로 이미 드러나서, 남겨 두면 한 구역에 라벨이 두 개 겹칩니다.
- 임시 미리보기 라우트(확인 후 삭제)에서 실제 `AssignmentToday`를 390px로 렌더해 (1) 책만 있는 숙제, (2) 책 + 질문 미션이 있고 완료된 숙제 두 경우를 시안과 나란히 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.

## "재미있었어?" → "책은 어땠어?" + 책장 목록형에 평가 스티커 + 책장 탭 아이콘을 책갈피로 (사용자 요청 3건, 스크린샷 3장)

- **평점을 묻는 말을 열린 질문으로**: 기록 남기기(`app/library/add`)와 기록 고치기(`components/record-edit-modal.tsx`)의 스티커 섹션 라벨이 **"재미있었어?"**였는데, "너무 단순하고 재미를 강요하는 느낌"이라는 지적으로 **"책은 어땠어?"**로 바꿨습니다 — 스티커 다섯 개(최고/재밌어/좋아/보통/별로) 중엔 "별로"도 있어서, 묻는 말이 이미 답을 정해 놓고 있으면 아이가 솔직하게 고르기 어렵습니다. 스티커 자체·저장 로직은 그대로입니다.
- **책장 목록형에 평가 스티커**: 표지 카드(전면 보기)에는 상태 배지가 있는데 목록 보기에는 평가가 전혀 안 보였습니다. `components/rating-picker.tsx`에 **`RatingSticker`**(작은 블롭 + 라벨, 고르는 화면과 같은 색·아이콘·이름)를 export로 추가하고, `components/library-shelf.tsx`의 목록 줄 오른쪽 — 날짜 바로 아래 — 에 붙였습니다. 표지 높이(44px) 안에 날짜(14px)+스티커(18px)가 들어가서 줄 높이는 그대로고, 평점을 안 남긴 기록은 예전처럼 날짜만 뜹니다. 360px에서 가장 긴 라벨("재밌어")까지 잘림 없이 들어가는 걸 실제 컴포넌트 렌더링으로 확인했습니다.
- **책장 탭 아이콘을 책갈피 모양으로 통일**: 하단 탭 "책장"(`components/icons/tab-icons.tsx`의 `LibraryIcon`)이 책등 세 권 모양이라, 표지 위 "책장에 꽂기" 토글(`components/read-toggles.tsx`의 `ShelfBookmark` → `misc-icons.tsx`의 `BookmarkIcon`)과 서로 다른 그림으로 같은 뜻("내 책장")을 가리키고 있었습니다. 탭 아이콘을 그 책갈피와 같은 path로 바꿔, 표지에서 책갈피를 눌러 꽂은 책이 어디로 가는지 아이콘만 봐도 이어지게 했습니다(`LibraryIcon`은 하단 탭에서만 쓰여서 다른 화면 영향 없음).
- 임시 미리보기 라우트에서 실제 `LibraryShelf` 목록 보기를 360/390px로 렌더해 스티커 다섯 종과 새 탭 아이콘을 스크린샷으로 확인했습니다(확인 후 삭제). `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.
...
## 책등을 표지에서 만들어 내기 + 책을 누를 때 펼쳐지는 애니메이션 (사용자 요청)

- **"책등을 실제 사진으로 넣을 수 있나"**: 카카오·알라딘·국립중앙도서관 어디도 책등 사진은 제공하지 않고 앞표지만 줍니다. 그래서 사용자가 고른 방식대로 **표지에서 책등을 만들어 냅니다** — `lib/shelf-visual.ts`의 `spineStyle(title, coverUrl)`이 표지 이미지를 CSS 배경으로 깔되 `background-size: auto 100%` + `background-position: left center`로 **표지 왼쪽 가장자리 띠**만 책등 높이에 맞춰 보이게 하고, 그 위에 옅은 어두운 막(그라데이션)을 덮어 흰 제목(`text-shadow` 추가)이 읽히게 합니다. 표지가 없으면 예전처럼 제목 해시 색. 처음엔 서버에서 색을 뽑아 `books`에 컬럼을 두려 했지만, CSS 배경은 카카오 CDN 같은 외부 도메인이어도 CORS·캔버스 오염 문제가 없어서 **마이그레이션 없이** 끝났습니다. 아이 책장(`components/library-shelf.tsx`)과 숲지기 추천도서 책등 보기(`components/operator-book-browser.tsx`) 둘 다 이 함수를 씁니다.
- **책등 한 줄 8권 → 7권**: 미리보기에서 360px 폰 기준 8번째 책이 카드 오른쪽 밖으로 잘리는 걸 발견했습니다. 예전 "8권 = 298px"는 책장이 배경 위에 바로 있을 때 계산한 값인데, 이후 책장 전체를 흰 카드(`p-4`) 안으로 넣으면서 안쪽 폭이 257px로 줄었는데 권수를 안 맞춘 회귀였습니다. 7권 + `gap-1`(251px)로 두 화면 모두 고쳤습니다.
- **책을 누를 때 애니메이션**(`app/globals.css`): 표지 보기는 왼쪽 등을 축으로 살짝 펼쳐지는 `book-open`(perspective + rotateY, 0.42s), 책등 보기는 선반에서 위로 뽑히는 `book-pull`, 목록 줄은 오른쪽으로 살짝 밀리는 `book-nudge`. `LibraryShelf`의 `openBook()`이 눌린 책의 recordId를 `opening`에 잠깐 들고 있다가 380ms 뒤 기록 고치기 모달을 엽니다(움직임 최소화 설정이면 즉시). 모달(`components/record-edit-modal.tsx`)도 배경 페이드(`backdrop-in`) + 시트가 아래에서 올라오는 `sheet-up`을 붙여, 이 모달을 여는 다른 화면(오늘 탭 최근 기록·숙제)에서도 같은 전환이 납니다. 전부 `prefers-reduced-motion`이면 꺼집니다.
- 임시 미리보기 라우트(확인 후 삭제)에서 실제 `LibraryShelf`를 360/390px로 렌더해 책등·탭 중간 프레임·모달을 스크린샷으로 확인했습니다. `npm run lint`/`rm -rf .next && npm run build` 통과. DB 변경 없음.
- **후속(사용자 스크린샷: "책등은 좀 더 어둡게, 표지 가운데를 따와야")**: 왼쪽 가장자리 띠는 실제 표지에서 여백·제본선·배경이 걸려 밋밋하고 글자도 안 읽혔습니다. `spineStyle()`의 `background-position`을 `center`로 바꿔 **표지 가운데 세로 띠**를 쓰고, 어두운 막을 0.22~0.42에서 **0.5~0.66**으로 진하게 올려 흰 제목이 확실히 읽히게 했습니다. 임시 미리보기(삭제)로 확인, lint·build 통과.
- **후속(사용자 결정: "블러 처리 중간")**: 진하게 해도 「100층짜리 집」처럼 표지 한가운데 큰 제목 글자가 있는 책은 그 글자가 우리가 얹은 흰 제목과 두 겹으로 겹쳐 지저분했습니다. 표지 띠에 **블러 2.5px**를 줘서 색·분위기만 남기고 글자·세부는 뭉개지게 했습니다(실제 책등을 멀리서 보는 느낌). CSS `filter`는 배경 이미지에 직접 못 걸어서 `spineStyle()`을 없애고 `components/spine-cover.tsx`(안쪽 절대배치 레이어: 블러된 표지 — 가장자리가 흐려져 비치지 않게 사방 6px 크게 그려 클리핑 — 위에 어두운 막)로 바꿨습니다. 값은 `lib/shelf-visual.ts`의 `SPINE_BLUR_PX`/`SPINE_SCRIM`. 책등 버튼에 `relative`, 제목 span에 `relative`를 줘 글자가 레이어 위에 오게 했습니다. 아이 책장·숲지기 추천도서 책등 보기 둘 다 적용. 임시 미리보기(삭제)로 확인, lint·build 통과.
- **후속(사용자 결정: "책등 사진 넣는 생각 다 삭제, 원래로. 그리고 폭에 맞춰 자동으로 채우기")**: 표지 띠(가운데·어두운 막·블러)까지 해 봤지만 흰 제목의 가독성이 계속 떨어져 **표지 책등을 전부 걷어냈습니다** — `components/spine-cover.tsx` 삭제, `SPINE_BLUR_PX`/`SPINE_SCRIM` 제거, 책등은 다시 제목 해시 팔레트 단색(`spineColor`)에 흰 제목뿐(text-shadow도 제거). 한 줄 권수는 **고정 7권 → 선반 폭에 맞춰 자동**: `components/use-spines-per-row.ts`(`ResizeObserver`로 선반 컨테이너 폭을 재 `lib/shelf-visual.ts`의 `spinesPerRow()`로 계산, 첫 렌더는 7권으로 시작해 튀지 않게)를 아이 책장·숲지기 추천도서 책등 보기 둘 다에 붙였습니다. 360px 폰에서 7권째가 1.5px 모자라 6권으로 떨어지길래 선반 줄 좌우 여백을 `px-3`→`px-2`로 줄여 **360px 7권 / 430px 9권**이 되는 걸 임시 미리보기(삭제)로 확인했습니다. lint·build 통과.
...
## 앱 아이콘 교체 — 사용자가 그린 밤 숲 위에 "책숲" (사용자 요청: "숲 가득한 배경에 책숲이라고 글씨 채운 아이콘")

예전 아이콘(`assets/icon-only.png`, 곰 발이 든 등불)은 **연한 바탕 한가운데 작은 초록 네모**로 그려져 있어서, iOS가 아이콘을 다시 둥근 네모로 잘라내면 홈 화면에서 "연한 테두리 안의 작은 아이콘"처럼 보이는 문제가 있었습니다(다른 앱보다 작고 흐릿하게 보임). 이번 교체로 그 문제도 같이 없어졌습니다.

- **후보 제시 과정**: 기존 크레용 조각(등불·곰·나무·발자국)으로 6안, 밤 숲 배경을 코드로 그려 "책숲" 글자를 얹은 3안을 스크린샷 시트로 보여 준 뒤, 사용자가 자기 그림(겹겹이 선 밤 숲, `docs/illustrations/icon-forest-src.jpg`, 671×773 — "내 그림이라 그냥 써도 됨")을 배경으로 쓰기로 했습니다. 글자는 스플래시 문구와 같은 명조 **Gowun Batang Regular**("Bold는 너무 두꺼워"), 크림색 + 옅은 그림자.
- **배경 손질(코드, PIL)**: 정사각 크롭 → 2048로 확대 → 아래 절반부터 점점 밝아지는 그라데이션 마스크로 앞쪽 나무를 밝은 연두로 → 하늘(남색 픽셀)에만 별 170개 → 오른쪽 위에 반달(초승달에 가깝게) + 은은한 빛 번짐. 손질된 배경은 `docs/illustrations/icon-forest-bg.png`에 보관(글자 없이 다시 쓸 때).
