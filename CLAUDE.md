...
- **기록 탭에서 상태 바꾸기**: `components/record-status.tsx`가 각 기록 카드에 3버튼 세그먼트 컨트롤을 붙여서, 나중에 "읽고 싶어요" → "읽는 중" → "다 읽음"으로 자유롭게 바꿀 수 있습니다. 읽던 책을 다 읽으면 그제서야 상태를 "다 읽음"으로 바꾸는 식으로 쓰면 됩니다(별도로 평점/메모를 그때 추가하는 UI는 아직 없고, 상태만 바뀝니다).
