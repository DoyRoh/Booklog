...
> **인텔 맥(예: 2020년형 iMac)도 됩니다.** macOS 26 Tahoe + Xcode 26이면 이 프로젝트의 모든 단계(시뮬레이터·Archive·TestFlight·AAB)가 동작합니다. 다만 Tahoe가 인텔 맥을 지원하는 마지막 macOS라, Apple이 "최신 SDK로 빌드한 앱만 제출"로 기준을 올리는 시점(보통 매년 4월경 — 2027년 봄 이후로 추정) 뒤에는 인텔 맥으로 스토어 업로드가 막힐 수 있습니다. 첫 출시엔 충분하고, 이 앱은 화면·기능 수정을 웹 배포만으로 반영하므로 재업로드가 필요한 일 자체가 드뭅니다. 여유 디스크 60GB 이상을 확보하고, 메모리 8GB면 Xcode와 Android Studio를 한 번에 하나씩만 여세요.
...
- **5.1.1 개인정보**: 아이 정보를 다루므로 개인정보 처리방침 URL이 반드시 있어야 하고(있음), 회원 탈퇴/계정 삭제 방법이 앱 안에 있어야 합니다(**5.1.1(v)**). 앱의 **프로필·설정 → 계정 → 계정 삭제하기**가 이 요건을 채웁니다(마이그레이션 0029 `delete_my_account()`가 Supabase에 적용돼 있어야 실제로 동작 — 심사 제출 전에 테스트 계정으로 한 번 눌러서 로그인 화면으로 돌아오는지 확인).
