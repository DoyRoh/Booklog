...
`supabase/migrations/0007_phase_storage.sql`을 SQL Editor에서 실행해야 합니다 — 비공개 Storage 버킷 `reading-media`를 만들고 RLS를 겁니다(버킷 생성도 SQL로 되고, 별도로 대시보드에서 만들 필요는 없습니다).
...
로컬 Postgres로 `storage.objects`를 최소 스키마로 모킹해 보호자는 자기 아이 폴더 전체를 CRUD, 교사는 낭독 미션 녹음만 조회(사진·음성 메모는 아예 접근 불가), 다른 부모는 완전 차단(조회 0건, 업로드 거부)까지 실제 쿼리로 확인했습니다.
